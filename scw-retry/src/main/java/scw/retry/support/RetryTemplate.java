...
import scw.retry.RecoveryCallback;
import scw.retry.RetryCallback;
import scw.retry.RetryContext;
import scw.retry.RetryException;
import scw.retry.RetryListener;
import scw.retry.RetryOperations;
import scw.retry.RetryPolicy;
import scw.retry.TerminatedRetryException;
import scw.retry.policy.SimpleRetryPolicy;

public class RetryTemplate implements RetryOperations {
	private static Logger logger = LoggerFactory
			.getLogger(RetryTemplate.class);
	...
	public void setThrowLastExceptionOnExhausted(
	...
	public <T> T execute(RetryCallback<T> retryCallback,
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Retry failed last attempt: count="
						+ context.getRetryCount());
			}
	 ...
	 *
	 * @param retryPolicy
	 *            a {@link RetryPolicy} to delegate the context creation
	 ...
	 *            the callback for recovery (might be null)
	 * @param context
	 *            the current retry context
	 * @throws Exception
	 *             if the callback does, and if there is no callback and the
	 *             state is null then the last exception from the context
	...
	protected <E extends Throwable> void rethrow(RetryContext context,
		...
		if (throwLastExceptionOnExhausted) {
			...
			throw new ExhaustedRetryException(message,
