...
public abstract class AbstractBeanDefinitionLoaderChain implements BeanDefinitionLoaderChain {
	private static Logger logger = LoggerFactory.getLogger(AbstractBeanDefinitionLoaderChain.class);
	...
	public final BeanDefinition load(ConfigurableBeanFactory beanFactory, Class<?> sourceClass) {
		...
		if (loader == null) {
			return chain == null ? null : chain.load(beanFactory, sourceClass);
		}
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("ignore bean builder loader [{}] ERROR [{}] Reason [{}]", loader.getClass(), e.getClass(),
						e.getMessage());
			}
