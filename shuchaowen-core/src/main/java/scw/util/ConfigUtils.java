...
public final class ConfigUtils {
	...
	public static <T> T parseObject(Map<String, String> map, Class<T> clz) throws Exception {
		...
		for (Entry<String, String> entry : map.entrySet()) {
			FieldContext fieldContext = MapperUtils.getFieldFactory().getFieldContext(clz, entry.getKey(), null, FilterFeature.SUPPORT_SETTER);
	...
	public static <T> T setProperties(Object obj, Properties properties, StringFormat stringFormat) {
		...
		try {
			for (Entry<Object, Object> entry : properties.entrySet()) {
				...
				FieldContext fieldContext = MapperUtils.getFieldFactory().getFieldContext(obj.getClass(), key, null);
				if (fieldContext == null) {
					continue;
				}
