...
public class LoggerLevelManager {
	...
	public Level getLevel(String name) {
		Level level = getLevel(DEFAULT_LEVEL_MAP, name);
		if (level == null && levelMap != null) {
			level = getLevel(levelMap, name);
		}
	...
	public final class DynamicLevel implements EventRegistration {
		...
		public DynamicLevel(final String name) {
			...
			this.eventRegistration = LoggerLevelManager.this.eventDispatcher.registerListener(new EventListener<BasicEvent>() {

				public void onEvent(BasicEvent event) {
					Level level = LoggerLevelManager.this.getLevel(name);
		...
		public BasicEventDispatcher<BasicEvent> getEventDispatcher() {
			if (eventDispatcher == null) {
				eventDispatcher = new DefaultBasicEventDispatcher<BasicEvent>(false);
			}
			return eventDispatcher;
		}
	}
