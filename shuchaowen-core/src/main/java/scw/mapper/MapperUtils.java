...
public class MapperUtils {
	...
	private static final Mapper MAPPER = new DefaultMapper(Arrays.asList(
			BOOLEAN_GETTER_METHOD_PREFIX, DEFAULT_GETTER_METHOD_PREFIX),
	...
	public static String getGetterMethodName(Field field) {
		...
		if (field.getType() == boolean.class) {
			if (name.length() > 2
					&& name.startsWith(BOOLEAN_GETTER_METHOD_PREFIX)
					&& Character.isUpperCase(name.charAt(2))) {
				return name;
			}

			return BOOLEAN_GETTER_METHOD_PREFIX
					+ StringUtils.toUpperCase(name, 0, 1);
		} else {
			return DEFAULT_GETTER_METHOD_PREFIX
					+ StringUtils.toUpperCase(name, 0, 1);
		}
	}
	...
	public static String getSetterMethodName(Field field) {
		...
		return DEFAULT_SETTER_METHOD_PREFIX
				+ StringUtils.toUpperCase(name, 0, 1);
	}

	public static void setStringValue(scw.mapper.Field field, Object instance,
			String value) {
		field.getSetter().set(instance,
				ValueUtils.parse(value, field.getSetter().getGenericType()));
	}
