...
public final class MultiFilter implements Filter {
	private Enumerable<Filter> enumerableFilters;

	public MultiFilter(Enumerable<Filter> enumerableFilters) {
		this.enumerableFilters = enumerableFilters;
	}

	public MultiFilter(NoArgsInstanceFactory instanceFactory,
			Collection<String> filterNames) {
		this(new Enumerable.InstanceNamesEnumerable<Filter>(instanceFactory,
				filterNames));
	}

	public MultiFilter(Collection<Filter> filters) {
		this(new Enumerable.CollectionEnumerable<Filter>(filters));
	}

	public Object doFilter(ProxyInvoker invoker, Object[] args)
			throws Throwable {
		return new FilterProxyInvoker(invoker, enumerableFilters.enumeration())
				.invoke(args);
	}

	public static final class FilterProxyInvoker extends ProxyInvokerWrapper {
		...
		public FilterProxyInvoker(ProxyInvoker invoker,
			...
			this(invoker, filters == null ? null : Collections
					.enumeration(filters));
		}
		...
		public FilterProxyInvoker(ProxyInvoker invoker,
				...
				Collection<String> filterNames) {
			this(invoker, new FilterNamesEnumeration(instanceFactory,
					filterNames));
		}
	...
	private static final class FilterNamesEnumeration implements
			Enumeration<Filter> {
