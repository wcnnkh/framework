...
public abstract class Aop implements ProxyAdapter {
	...
	public Proxy proxy(Class<?> clazz, Class<?>[] interfaces,
		...
		return getProxyAdapter().proxy(clazz, interfaces, getFilters(),
	...
	public final <T> Proxy proxyInstance(Class<? extends T> clazz, T instance,
		...
		return proxy(clazz, interfaces,
				Arrays.asList(new InstanceFilter(instance)),
				new DefaultFilterChain(filters, filterChain));
	}
	
	public final Proxy proxy(Class<?> clazz, Class<?>[] interfaces,
			Collection<? extends Filter> filters) {
		return proxy(clazz, interfaces, filters, null);
	}

	public final <T> MethodInvoker proxyMethod(Class<? extends T> targetClass,
			T instance, Method method, Collection<? extends Filter> filters,
			FilterChain filterChain) {
		return new DefaultProxyInvoker(targetClass, instance, method,
				getFilters(), new DefaultFilterChain(filters, filterChain));
	}
	...
	public final MethodInvoker proxyMethod(
		...
		return new InstanceFactoryProxyInvoker(noArgsInstanceFactory,
				instanceName, targetClass, method, filters, filterChain);
	}
	...
	public final MethodInvoker proxyMethod(
		...
		return proxyMethod(noArgsInstanceFactory, targetClass.getName(),
				targetClass, method, filters, filterChain);
	}
	...
	public final MethodInvoker proxyMethod(
			...
			Method method, Collection<? extends Filter> filters) {
		return proxyMethod(noArgsInstanceFactory, targetClass.getName(),
				targetClass, method, filters, null);
	}
	...
	private static abstract class ProxyInvoker extends MethodHolderInvoker {
		...
		public Object invoke(Object... args) throws Throwable {
			...
			return invoke.doFilter(getInvoker(bean), bean, getMethodHolder()
