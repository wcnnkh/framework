...
public class ContentCachingResponseWrapper extends ServerHttpResponseWrapper{
	...
	private final OutputStream outputStream = new ResponseOutputStream();
	...
	public ContentCachingResponseWrapper(ServerHttpResponse response) {
		super(response);
	}
	...
	public void setStatusCode(HttpStatus httpStatus) {
		super.setStatusCode(httpStatus);
	...
	public PrintWriter getWriter() throws IOException {
		if (this.writer == null) {
			...
			this.writer = (characterEncoding != null ? new ResponsePrintWriter(characterEncoding) :
					new ResponsePrintWriter(Constants.DEFAULT_CHARSET_NAME));
		}
		return this.writer;
	}
	...
	protected void copyBodyToResponse(boolean complete) throws IOException {
		if (this.content.size() > 0) {
			if ((complete || this.contentLength != null) && !targetResponse.isCommitted()) {
				targetResponse.setContentLength(complete ? this.content.size() : this.contentLength);
				this.contentLength = null;
			}
			this.content.writeTo(targetResponse.getBody());
			this.content.reset();
			if (complete) {
				super.flush();
			}
