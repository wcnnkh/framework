...
public class JedisPipeline implements RedisPipeline<byte[], byte[]> {
	private final Pipeline pipeline;

	public JedisPipeline(Pipeline pipeline) {
		this.pipeline = pipeline;
	}

	@Override
	public RedisResponse<Long> geoadd(byte[] key, GeoaddOption option, Map<byte[], Point> members) {
		return new DefaultRedisResponse<Long>(() -> pipeline
				.geoadd(key, JedisUtils.toGeoAddParams(option), JedisUtils.toMemberCoordinateMap(members)).get());
	}
