...
public interface ToBinaryEncoder<D> extends Encoder<D, byte[]> {
	...
	default <E extends Throwable> void encodeToStream(D source, @NonNull OutputStream target)
			throws IOException, CodecException {
		try (WritableByteChannel channel = Channels.newChannel(target)) {
			encodeToChannel(source, channel);
		}
	}

	default <E extends Throwable> void encodeToChannel(D source, @NonNull WritableByteChannel target)
			throws IOException, CodecException {
		encodeToConsumer(source, target::write);
	}

	<E extends Throwable> void encodeToConsumer(D source,
			@NonNull ThrowingConsumer<? super ByteBuffer, ? extends E> target) throws CodecException, E;
