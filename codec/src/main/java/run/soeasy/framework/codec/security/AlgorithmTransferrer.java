...
public abstract class AlgorithmTransferrer<T> implements BinaryTransferrer {
    ...
    @Override
    public void transfer(@NonNull BufferReader<? super ByteBuffer> reader, @NonNull BufferWriter<? super ByteBuffer> writer) throws IOException {
        try {
            T algorithm = this.algorithmFactory.getAlgorithm();
            execute(algorithm, reader, writer);
        } catch (GeneralSecurityException e) {
            throw new CodecException(e);
        }
    }

    public abstract void execute(T algorithm, @NonNull BufferReader<? super ByteBuffer> reader, @NonNull BufferWriter<? super ByteBuffer> writer) throws IOException, GeneralSecurityException;
}