...
public abstract class Aop implements ProxyFactory {
	...
	public final Proxy getProxy(Class<?> clazz, Class<?>[] interfaces,
	...
	public final Proxy getProxy(Class<?> clazz, Class<?>[] interfaces,
			Collection<MethodInterceptor> filters) {
		return getProxy(clazz, interfaces,
				(Iterable<MethodInterceptor>) filters);
	}
	...
	public final Proxy getProxy(Class<?> clazz, Class<?>[] interfaces,
		...
		return getProxy(clazz, interfaces,
				new InstanceIterable<MethodInterceptor>(instanceFactory,
						filterNames));
	}
	...
	public final Class<?> getUserClass(String className, boolean initialize,
	...
	public final <T> Proxy getProxyInstance(Class<? extends T> clazz,
			T instance, Class<?>[] interfaces, MethodInterceptor... filters) {
		return getProxyInstance(clazz, instance, interfaces,
				Arrays.asList(filters));
	}

	@SuppressWarnings("unchecked")
	private final MethodInvoker wrapper(MethodInvoker invoker,
			Iterable<? extends MethodInterceptor> filters, boolean singletion) {
		if (isProxyMethod(invoker.getInstance(), invoker.getMethod())) {
			return new ProxyMethodInvoker(invoker, filters, singletion);
		}
		return new ProxyMethodInvoker(invoker,
				new MultiIterable<MethodInterceptor>(getFilters(), filters), singletion);
	}
	 ...
	 * @return
	 */
	public final <T> MethodInvoker getProxyMethod(
			Class<? extends T> targetClass, T instance, Method method,
			Iterable<? extends MethodInterceptor> filters) {
		MethodInvoker proxyInvoker = new DefaultMethodInvoker(instance,
				targetClass, method);
	...
	public final <T> MethodInvoker getProxyMethod(
			Class<? extends T> targetClass, T instance, Method method,
			MethodInterceptor... filters) {
		return getProxyMethod(targetClass, instance, method,
				Arrays.asList(filters));
	}
	...
	public final MethodInvoker getProxyMethod(
		...
		if (!instanceFactory.isInstance(instanceName)) {
			throw new NotSupportedException("instanceName:" + instanceName
		...
		MethodInvoker methodInvoker = new DefaultMethodInvoker(instanceFactory,
				instanceName, targetClass, method, true);
	...
	public final MethodInvoker getProxyMethod(
		...
		return getProxyMethod(instanceFactory, instanceName, targetClass,
				method, Arrays.asList(filters));
	}
	...
	public final MethodInvoker getProxyMethod(
		...
		return getProxyMethod(noArgsInstanceFactory, targetClass, method,
				Arrays.asList(filters));
	}
	...
	private static final class InstanceMethodInterceptor implements
		...
		public Object intercept(MethodInvoker invoker, Object[] args,
				MethodInterceptorChain filterChain) throws Throwable {
			MethodInvoker proxyInvoker = new DefaultMethodInvoker(instance,
					invoker.getSourceClass(), invoker.getMethod(), true);
			return filterChain.intercept(proxyInvoker, args);
		}
	}

	private static class ProxyMethodInvoker extends MethodInvokerWrapper
		...
		public Object invoke(Object... args) throws Throwable {
			MethodInvoker invoker = singletion ? getSource()
					: new DefaultMethodInvoker(getInstance(), getSourceClass(),
							getMethod());
			if (filters == null) {
				return invoker.invoke(args);
			}
			return new MethodInterceptorChain(filters.iterator()).intercept(
					invoker, args);
		}
	}
}
