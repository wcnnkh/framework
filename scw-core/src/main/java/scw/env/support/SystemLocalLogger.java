...
import scw.io.support.LocalLogger;
...
public final class SystemLocalLogger<T> {
	private static final String DIRCETORY = SystemEnvironment.getInstance().getValue("scw.system.local.directory",
			String.class, SystemEnvironment.getInstance().getUserHome());
	...
	public SystemLocalLogger(String name, NoTypeSpecifiedSerializer serializer) {
		this.localLogger = new LocalLogger(StringUtils
				.cleanPath(DIRCETORY + "/" + SystemEnvironment.getInstance().getPrivateId() + "/" + name));
