...
public abstract class AbstractSockJsService implements SockJsService {
	...
	public AbstractSockJsService(ScheduledExecutorService scheduledExecutorService) {
		Assert.notNull(scheduledExecutorService, "TaskScheduler must not be null");
		...
		try {
			if (sockJsPath.isEmpty() || sockJsPath.equals("/")) {
				...
				if (isWebSocketEnabled()) {
					...
					handleRawWebSocketRequest(httpChannel, wsHandler);
				}
				...
				handleTransportRequest(httpChannel, wsHandler, sessionId, transport);
			}
			//TODO response.close();
		}
	...
	protected boolean checkOrigin(ServerHttpRequest request, ServerHttpResponse response, HttpMethod... httpMethods)
		...
		if (HttpUtils.isSameOrigin(request)) {
			return true;
		}

		if (!HttpUtils.isValidOrigin(request, this.allowedOrigins)) {
			if (logger.isWarnEnabled()) {
	 ...
	 */
	protected abstract void handleRawWebSocketRequest(HttpChannel httpChannel, WebSocketHandler webSocketHandler) throws IOException;

	/**
	 * Handle a SockJS session URL (i.e. transport-specific request).
	 */
	protected abstract void handleTransportRequest(HttpChannel httpChannel,
