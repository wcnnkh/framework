...
import scw.websocket.sockjs.SockJsMessageDeliveryException;
...
public abstract class AbstractSockJsSession implements SockJsSession {
	...
	public static final String DISCONNECTED_CLIENT_LOG_CATEGORY =
			"scw.websocket.sockjs.DisconnectedClient";
	 ...
	 */
	protected static final Logger disconnectedClientLogger = LoggerUtils.getLogger(DISCONNECTED_CLIENT_LOG_CATEGORY);
	...
	protected void scheduleHeartbeat() {
		...
		synchronized (this.responseLock) {
			...
			this.heartbeatFuture = this.config.getScheduledExecutorService().schedule(this.heartbeatTask, config.getHeartbeatTime(), TimeUnit.MILLISECONDS);
