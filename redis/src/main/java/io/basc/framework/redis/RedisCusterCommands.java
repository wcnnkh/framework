...
public interface RedisCusterCommands {
	 ...
	 * 
	 * @param slots
	 * @return Simple string reply: OK if the command was successful. Otherwise an
	 *         error is returned.
	 */
	 ...
	 * 
	 * @return Simple string reply: BUMPED if the epoch was incremented, or STILL if
	 *         the node already has the greatest config epoch in the cluster.
	 */
	 ...
	 * serving the specified hash slot will always result in a count of zero being
	 * returned.
	 * 
	 * @param slot
	 * @return Integer reply: The number of keys in the specified hash slot, or an
	 *         error if the hash slot is invalid.
	 */
	 ...
	 *         failover is going to be attempted. An error if the operation cannot
	 *         be executed, for example if we are talking with a node which is
	 *         already a master.
	 */
	 ...
	 * In other words the specified node is removed from the nodes table of the node
	 ...
	 * Cluster specification, or in a more simple to digest form, as an appendix of
	 * the CLUSTER SETSLOT command documentation.
	 * 
	 * @param slot
	 * @param count
	 ...
	 * 
	 * @return Bulk string reply: A map between named fields and values in the form
	 *         of <field>:<value> lines separated by newlines composed by the two
	 *         bytes CRLF.
	 */
	byte[] cluster_info();
	 ...
	 * This command is mainly useful for debugging and testing, since it exposes via
	 * an API the underlying Redis implementation of the hashing algorithm. Example
	 * use cases for this command:
	 * 
	 * Client libraries may use Redis in order to test their own hashing algorithm,
	 ...
	 * same. Humans may use this command in order to check what is the hash slot,
	 * and then the associated Redis Cluster node, responsible for a given key.
	 * 
	 * @param key
	 ...
	 * @param ip
	 * @param port
	 * @return Simple string reply: OK if the command was successful. If the address
	 ...
	 *         Starting with Redis 5, if not for backward compatibility, the Redis
	 *         project no longer uses the word slave. Unfortunately in this command
	 *         the word slave is part of the protocol, so we'll be able to remove
	 *         such occurrences only when this API will be naturally deprecated.
	 */
	 ...
	 * configuration update, it may show stale information. However eventually (in a
	 * matter of seconds if there are no network partitions) all the nodes will
	 * agree about the set of nodes associated with a given master.
	 * 
	 * @param nodeId
	 ...
	 * 
	 * Read queries against a Redis Cluster slave node are disabled by default, but
	 * you can use the READONLY command to change this behavior on a per- connection
