...
public interface RedisConnection<K, V> extends RedisCommands<K, V>, AutoCloseable {
	...
	/**
	 * Activates the pipeline mode for this connection. When pipelined, all commands
	 * return null (the reply is read at the end through {@link #closePipeline()}.
	 * Calling this method when the connection is already pipelined has no effect.
	 * Pipelining is used for issuing commands without requesting the response right
	 * away but rather at the end of the batch. While somewhat similar to MULTI,
	 * pipelining does not guarantee atomicity - it only tries to improve
	 * performance when issuing a lot of commands (such as in batching scenarios).
	 * <p>
	 * Note:
	 * </p>
	 * Consider doing some performance testing before using this feature since in
