...
public abstract class StandardSqlDialect extends DefaultTableMapper implements SqlDialect {
	...
	public Sql toSql(Operation operation) {
		...
		if (operation instanceof QueryOperation) {
			...
			appendOrderBy(sb, params, queryOperation.getOrders());
		} else {
			OperationSymbol operationSymbol = operation.getOperationSymbol();
			if (operationSymbol instanceof UpdateOperationSymbol) {
				sb.append("update ");
				keywordProcessing(sb, operation.getRepository().getName());
				sb.append(" set ");
