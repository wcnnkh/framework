...
public interface MessageConverter extends MessageReader, MessageWriter {
     ...
     * 获取当前转换器支持的所有媒体类型
     * 
     * <p>返回包含该转换器能够处理的所有媒体类型（如"application/json"、"text/plain"等），
     * 用于快速判断转换器是否适用于特定媒体类型的消息。
     * 
     * @return 支持的媒体类型集合（非空，可能为空集合）
     */
	Streamable<MediaType> getSupportedMediaTypes();

    /**
     * 根据指定的类型描述符和消息，返回当前转换器支持的媒体类型子集
     * 
     * <p>该方法会判断转换器是否支持对指定类型（{@link AccessibleDescriptor}）进行读写操作，
     * 若支持则返回{@link #getSupportedMediaTypes()}，否则返回空集合（{@link Streamable#empty()}）。
     * 
     * @param requiredDescriptor 类型描述符（包含待转换的类型信息，非空）
     * @param message 关联的消息（非空）
     * @return 支持的媒体类型集合（非空，要么是{@link #getSupportedMediaTypes()}的结果，要么是{@link Streamable#empty()}）
     */
    default Streamable<MediaType> getSupportedMediaTypes(@NonNull AccessibleDescriptor requiredDescriptor, @NonNull Message message) {
