...
public class TextMessageConverter extends AbstractTextMessageConverter<Object> implements ConverterAware {
     ...
     * 
     * <p>支持性条件：
     * 1. 转换器（{@link #converter}）能够将字符串（{@link String}）转换为目标类型；
     * 2. 父类的媒体类型匹配检查通过（{@link AbstractTextMessageConverter#isReadable}）。
     * 
    ...
    public boolean isReadable(@NonNull TargetDescriptor targetDescriptor, @NonNull Message message,
        ...
        return getConverter().canConvert(
     ...
     * 
     * <p>支持性条件：
     * 1. 转换器（{@link #converter}）能够将源类型转换为字符串（{@link String}）；
     * 2. 父类的媒体类型匹配检查通过（{@link AbstractTextMessageConverter#isWriteable}）。
     * 
    ...
    public boolean isWriteable(@NonNull SourceDescriptor sourceDescriptor, @NonNull Message message,
        ...
        return getConverter().canConvert(
                sourceDescriptor.getReturnTypeDescriptor(),
                TypeDescriptor.valueOf(String.class)
     ...
     * 将文本字符串转换为目标类型的对象
     * 
     * <p>通过{@link #converter}将字符串转换为目标描述符指定的类型（如将"123"转换为Integer，"2024-01-01"转换为LocalDate）。
     * 
     ...
     */
    @Override
    protected Object parseObject(String body, TargetDescriptor targetDescriptor, @NonNull Charset charset) throws IOException {
     ...
     * 将源对象转换为文本字符串
     * 
     * <p>通过{@link #converter}将源对象转换为字符串（如将Integer转换为"123"，LocalDate转换为"2024-01-01"）。
     * 
     ...
     */
    @Override
    protected String toString(TypedData<Object> body, MediaType contentType, @NonNull Charset charset) throws IOException {
        return (String) getConverter().convert(
                body.value(),
                TypeDescriptor.forObject(body.value()),
                TypeDescriptor.valueOf(String.class)
            );
    }
}