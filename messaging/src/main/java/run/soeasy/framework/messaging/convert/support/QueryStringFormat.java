 ...
 * 
 * <p>
 * 核心特性： - 使用"&amp;"作为键值对分隔符，"="作为键值分隔符，符合URL查询字符串规范； -
...
public class QueryStringFormat extends BeanFormat {
	 ...
	 */
	public static final MapContainer<String, QueryStringFormat, ConcurrentHashMap<String, QueryStringFormat>> REGISTRY = new MapContainer<>(
	 ...
	 * 
	 * <p>
	 * 父类构造参数说明： - 键值对分隔符："&amp;"（如key1=val1&amp;key2=val2）； -
	 * 键值分隔符："="（如key=value）； - 编码器与解码器：均使用传入的{@link URLCodec}，确保键和值的URL安全编码。
	 * 
	 ...
	 * 
	 * <p>
	 * 简化版getFormat，适用于仅需按字符集区分的场景（大多数情况）。
	 * 
	...
	public static QueryStringFormat getFormat(@NonNull Charset charset) {
		return REGISTRY.getContainer().computeIfAbsent(charset.name(), (key) -> new QueryStringFormat(charset));
	}
	 ...
	 * 
	 * <p>
	 * 示例：将User对象{id=1, name="test"}转换为"id=1&amp;name=test"（特殊字符会URL编码）。
	 * 
	 ...
	 * 
	 * <p>
	 * 示例：将"id=1&amp;name=test"解析为User对象{id=1, name="test"}（自动解码URL编码的字符）。
	 * 
