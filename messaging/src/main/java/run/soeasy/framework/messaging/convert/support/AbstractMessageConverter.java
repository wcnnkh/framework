...
/**
 * 消息转换器抽象基类，实现{@link MessageConverter}接口并提供通用功能，
 * 为具体消息转换器实现提供基础骨架，减少重复代码。
 * 
 * <p>该类通过{@link MediaTypeRegistry}管理支持的媒体类型，
...
public abstract class AbstractMessageConverter implements MessageConverter {
     ...
     * 
     * <p>该方法将输入消息的内容（如输入流中的字节）转换为目标类型的对象，
     * 具体转换逻辑（如JSON反序列化、XML解析等）由子类实现。
     * 
     ...
     * 
     * <p>该方法将源对象转换为指定媒体类型的内容（如JSON字符串、XML字节流），
     ...
     * 获取当前转换器支持的所有媒体类型
     * 
     * <p>直接返回{@link #mediaTypeRegistry}，委托注册表提供支持的媒体类型集合。
     * 
     ...
     * 
     * <p>复制的信息包括：
     * - Content-Length：内容长度（若输出消息已设置）；
     * - Content-Type：内容类型（从输入消息复制）；
     ...
     * 
     * <p>判断逻辑：
     * - 若{@code contentType}为null（未指定），默认返回true；
     ...
     * 
     * <p>步骤：
     * 1. 确定实际使用的媒体类型（{@code contentTypeToUse}）：若{@code contentType}为null，使用注册表中的第一个媒体类型；
     ...
     * 
     * <p>步骤：
     * 1. 确定实际使用的媒体类型（{@code contentTypeToUse}）：若{@code contentType}为null，从注册表中筛选非通配符类型的第一个媒体类型；
     * 2. 若输出消息未设置Content-Type，将{@code contentTypeToUse}设为其Content-Type；
     * 3. 调用{@link #doWrite}执行具体写入。
     * 
