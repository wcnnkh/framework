...
public class MessageConverters extends ConfigurableServices<MessageConverter> implements MessageConverter {
	 ...
	 * 
	 * <p>
	 * 采用双重检查锁定实现线程安全的懒加载，确保全局仅初始化一次，预注册以下转换器： -
	 ...
	 * 
	 * <p>
	 * 初始化逻辑： 1. 设置服务接口为{@link MessageConverter}，限定管理的服务类型； 2.
	 * 配置排序器为{@link MessageConverterComparator#DEFAULT}，确保转换器按媒体类型特异性排序； 3.
	 * 注册注入器：当转换器实现{@link MessageConverterAware}时，自动注入当前集合实例。
	 */
	public MessageConverters() {
		super(MessageConverterComparator.DEFAULT);
	 ...
	 * 
	 * <p>
	 * 通过flatMap操作合并所有转换器的{@link #getSupportedMediaTypes()}结果，
	 * 形成全局支持的媒体类型集合，用于快速判断系统是否支持某种媒体类型。
	 * 
	 * @return 合并后的媒体类型集合（非空，包含所有转换器支持的类型）
	 */
	@Override
	public Streamable<MediaType> getSupportedMediaTypes() {
		return flatMap((converter) -> converter.getSupportedMediaTypes().stream());
	}
	 ...
	 * 
	 * <p>
	 * 根据目标类型动态筛选支持的媒体类型，仅返回能处理该类型的转换器所支持的媒体类型，
	 ...
	 */
	@Override
	public Streamable<MediaType> getSupportedMediaTypes(@NonNull AccessibleDescriptor requiredDescriptor,
			@NonNull Message message) {
		return flatMap((converter) -> converter.getSupportedMediaTypes(requiredDescriptor, message).stream());
	}
	 ...
	 * 
	 * <p>
	 * 遍历所有转换器，检查是否有转换器支持{@code targetDescriptor}类型和{@code contentType}媒体类型，
	...
	public boolean isReadable(@NonNull TargetDescriptor targetDescriptor, @NonNull Message message,
		...
		return anyMatch((converter) -> converter.isReadable(targetDescriptor, message, contentType));
	}
	 ...
	 * 
	 * <p>
	 * 按排序后的顺序遍历转换器，选择第一个支持{@code targetDescriptor}和{@code contentType}的转换器执行读取操作，
	 ...
	 * 
	 * <p>
	 * 遍历所有转换器，检查是否有转换器支持{@code sourceDescriptor}类型和{@code contentType}媒体类型，
	...
	public boolean isWriteable(@NonNull SourceDescriptor sourceDescriptor, @NonNull Message message,
			MimeType contentType) {
		return anyMatch((converter) -> converter.isWriteable(sourceDescriptor, message, contentType));
	}
	 ...
	 * 
	 * <p>
	 * 按排序后的顺序遍历转换器，选择第一个支持{@code source}类型和{@code contentType}的转换器执行写入操作，
