 ...
 * 自动路由到对应的子映射器（{@link StreamableMapper} 或 {@link MappedMapper}），同时原生支持单键值对映射，
 ...
 * <li>启下：内部封装{@link StreamableMapper}和{@link MappedMapper}，根据上下文类型动态选择具体映射器，屏蔽场景差异；</li>
 ...
 *       创建{@link MappedMapper}实例（传入当前过滤器和this作为基础映射器），调用其{@code doMapping}处理“一键多值”映射；
 *     - 子策略2（数组集合）：若源不是Map类型，创建{@link StreamableMapper}实例（参数同上），调用其{@code doMapping}处理“顺序匹配”映射；
	...
	protected boolean doFilterableMapping(@NonNull MappingContext<K, V, T> sourceContext,
		...
		// 场景2：集合映射（源和目标均为映射集合）
			...
			if (sourceContext.getMapping().isMapped()) {
				...
				StreamableMapper<K, V, T> arrayMapper = new StreamableMapper<>(filters, this);
