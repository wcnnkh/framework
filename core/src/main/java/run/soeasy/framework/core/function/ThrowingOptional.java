...
public interface ThrowingOptional<T, E extends Throwable> extends ThrowingSupplier<T, E> {
    ...
    /**
     * 从值创建ThrowingOptional实例。
     * 如果值为null，则返回空的ThrowingOptional；否则返回包含该值的实例。
     *
     * @param <U>   值的类型
     * @param <E>   异常类型
    ...
    /**
     * 获取空的ThrowingOptional实例。
     * 空实例表示值不存在，调用get()方法会抛出NoSuchElementException。
     *
     * @param <U> 值的类型
     * @param <E> 异常类型
    ...
    /**
     * 过滤值，根据谓词条件决定是否保留值。
     * 如果谓词测试返回true，则保留值；否则将值置为null（转换为空的Optional）。
     *
    ...
    /**
     * 获取值，如果值不存在则抛出NoSuchElementException。
     * 该方法会先调用orElse(null)获取值，若值为null则抛出异常。
     *
     * @return 存在的值
     * @throws E                可能抛出的异常
    ...
    /**
     * 对值进行扁平映射转换，返回映射后的结果。
     * 该方法允许将当前Optional中的值转换为另一种类型，可能抛出新的异常类型。
     *
     * @param <R>    映射后的结果类型
     * @param <X>    可能抛出的新异常类型
    ...
    /**
     * 如果值存在，则执行指定的消费操作。
     * 该方法会先检查值是否存在，存在则执行consumer，否则不执行任何操作。
     *
    ...
    /**
     * 检查值是否存在。
     * 通过调用orElse(null)判断返回值是否为null来确定值是否存在。
     *
    ...
    /**
     * 对值进行映射转换，返回新的ThrowingOptional。
     * 该方法支持函数式编程的map操作，将T类型的值转换为R类型的值。
     *
     * @param <R>    映射后的结果类型
     * @param mapper 映射函数，不可为null
    ...
    /**
     * 获取值，如果值不存在则返回指定的默认值。
     * 该方法通过flatMap实现，如果值存在则返回值，否则返回other。
     *
     * @param other 默认值
     * @return 值或默认值
     * @throws E 可能抛出的异常
    ...
    /**
     * 获取值，如果值不存在则通过供应者获取默认值。
     * 该方法在值不存在时调用供应者，避免不必要的默认值创建。
     *
    ...
    /**
     * 返回自身，用于链式调用时保持接口一致性。
     * 由于当前对象已经是ThrowingOptional，直接返回自身。
     *
