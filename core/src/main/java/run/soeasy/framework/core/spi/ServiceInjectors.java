 ...
 * <p>
 * 该容器继承自{@link ServiceRegistry}，专门用于存储和管理{@link ServiceInjector}实例，
 * 实现了{@link ServiceInjector}接口以支持将服务实例注入到所有注册的注入器中， 形成责任链模式的服务注入机制。
 *
 * <p>
 * <b>核心特性：</b>
 * <ul>
 * <li>批量注入：调用{@link #inject}时，所有注册的{@link ServiceInjector}都会执行注入</li>
 * <li>有序执行：继承自{@link ServiceRegistry}，支持按顺序执行注入器</li>
 ...
 * @see ServiceInjector
 */
public class ServiceInjectors<S> extends ServiceRegistry<ServiceInjector<? super S>> implements ServiceInjector<S> {

	public ServiceInjectors() {
	...
	public Operation inject(S service) {
		return Operation.batch(this, Mode.AND, (e) -> e.inject(service));
	}
}