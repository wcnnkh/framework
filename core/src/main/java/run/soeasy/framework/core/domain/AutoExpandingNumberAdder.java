 ...
 * 
 * @author soeasy.run
 * @see NumberAdder 累加器基础契约接口
 * @see IntValue int类型累加器（最小范围）
 * @see LongValue long类型累加器（中等范围）
 * @see BigIntegerValue 高精度整数累加器（无范围限制）
 */
public class AutoExpandingNumberAdder extends NumberAdder {
	private static final long serialVersionUID = 1L;
	
	/** 
	 * 当前活跃的数值累加器实例，承载实际的数值存储与运算（动态切换类型：IntValue → LongValue → BigIntegerValue）
	 */
	private NumberAdder current;
	
	/** 
	 * 初始数值，用于{@link #reset()}时恢复到初始状态（含初始类型），不可修改
	 */
	 ...
	 * 
	 * @param initialValue 初始数值（用于后续reset恢复，不可为null）
	 * @param initialAdder 初始类型的{@link NumberAdder}实例（如int值对应{@link IntValue}，不可为null）
	 */
	 ...
	 * <p>初始类型固定为{@link IntValue}，因int值必然在自身范围内，无需降级。
	 * 
	 ...
	 * 避免直接使用高精度类型带来的性能损耗。
	 * 
	 * @param initialValue 初始BigInteger值（无范围限制，不可为null）
	 ...
	 * 
	 * @param value 待适配的BigInteger值（不可为null）
	...
	/**
	 * 取反操作：委托当前活跃的累加器执行取反逻辑
	 * <p>取反结果的类型与当前累加器类型一致，无溢出风险（若为BigIntegerValue）或触发类型升级（若为IntValue/LongValue）。
	 * 
	 * @return 取反后的{@link NumberValue}实例（原实例状态不变）
	 * @throws ArithmeticException 若取反过程中发生溢出（如{@link Integer#MIN_VALUE}取反），会触发类型升级
	 */
	@Override
