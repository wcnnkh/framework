...
public class EventDispatcher<T> extends AbstractChannel<T> implements ListenableChannel<T> {
	 ...
	 * 无参构造函数，使用默认配置
	 * <p>
	 * 初始化时使用{@link CopyOnWriteArrayList}作为监听器注册表，
	 ...
	 * 获取批量事件处理通道
	 * <p>
	 * 返回一个伪批量通道，实际操作会委派给当前实例， 适用于需要批量处理接口的场景。
	 * 
	 * @return 批量事件处理通道
	 */
	@Override
	 ...
	 * 注册事件监听器
	 * <p>
	 * 将监听器添加到注册表中，并返回可用于注销的注册句柄。 监听器会在事件发布时根据过滤器条件被调用。
	 * 
	 ...
	 */
	@Override
	public Operation registerListener(Listener<T> listener) {
	 ...
	 * 同步发布事件到所有匹配的监听器
	 * <p>
	 * 该方法会阻塞当前线程直到所有监听器处理完毕， 适用于事件处理耗时短、需要立即获取反馈的场景。
	 * 
	 * @param resource 要发布的事件对象，不可为null
	 * @throws NullPointerException 若resource为null
	 */
	public void syncPublish(T resource) {
		Streamable<Listener<? super T>> elements = filter.apply(registry);
		elements.consume((stream) -> CollectionUtils.acceptAll(stream.iterator(), (e) -> e.accept(resource)));
	}
}