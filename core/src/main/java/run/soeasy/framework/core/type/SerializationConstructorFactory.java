...
/**
 * 使用反射工厂创建序列化构造器的工厂类。 该工厂通过sun.reflect.ReflectionFactory创建特殊的构造器，
 * 允许在反序列化时绕过目标类的构造函数，直接创建对象实例， 主要用于需要自定义序列化逻辑的场景。
 *
 * <p>
 * 核心特性：
 * <ul>
...
public class SerializationConstructorFactory implements ClassMemberFactory<Constructor<?>> {
	...
	/**
	 * 获取ReflectionFactory类的Class对象。 使用双重检查锁实现延迟初始化，确保线程安全。
	 *
	...
	/**
	 * 获取ReflectionFactory类的newConstructorForSerialization方法。 使用双重检查锁实现延迟初始化，确保线程安全。
	 *
	...
	/**
	 * 获取ReflectionFactory类的实例。 使用双重检查锁实现延迟初始化，确保线程安全。
	 *
		...
		if (reflectionFactory == null && getReflectionFactoryDeclaringClass() != null) {
			synchronized (this) {
				...
				if (reflectionFactory == null && getReflectionFactoryDeclaringClass() != null) {
					...
					try {
						getReflectionFactoryMethod = getReflectionFactoryDeclaringClass()
	...
	/**
	 * 获取用于序列化的构造器提供者。 该方法使用ReflectionFactory创建一个特殊构造器， 该构造器可以在不执行目标类构造函数的情况下创建对象。
	 *
	...
	public Streamable<Constructor<?>> getClassMemberProvider(@NonNull Class<?> declaringClass) {
		if (getReflectionFactory() != null && getNewConstructorForSerialization() != null) {
			...
			if (constructor != null) {
				// 缓存构造器，避免重复创建
				return Streamable.singleton(constructor);
			}
		}
