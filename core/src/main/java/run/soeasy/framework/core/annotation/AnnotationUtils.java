...
import run.soeasy.framework.core.BridgeMethodResolver;
import run.soeasy.framework.core.annotation.AnnotationTypeMapping.MirrorSets.MirrorSet;
...
import run.soeasy.framework.util.JavaVersion;
...
public abstract class AnnotationUtils {
	 ...
	 * @return the first matching annotation, or {@code null} if not found
	 * @see run.soeasy.framework.core.BridgeMethodResolver#findBridgedMethod(Method)
	 ...
	 *                            {@link run.soeasy.framework.core.type.AnnotationMetadata})
	 ...
	 * proxy). If that fails, an attempt will be made to invoke the method via
	 * reflection.
	 * 
	...
	static Object invokeAnnotationMethod(Method method, Object annotation) {
		...
		return ReflectionUtils.invoke(method, annotation);
	}
	...
	/**
	 * Determine if the supplied {@link Annotation} has been <em>synthesized</em> by
	 * Spring (i.e. wrapped in a dynamic proxy) with additional functionality such
	 * as attribute alias handling.
	 * 
	 * @param annotation the annotation to check
	...
	public static boolean isSupported(@NonNull JavaVersion javaVersion,
		...
		if (!CollectionUtils.isEmpty(map)) {
			...
			if (!javaVersion.isSupported(version)) {
				return false;
			}
		}
		return true;
