 ...
 * {@link DefaultResolvableTypeFactory}、{@link ResolvableTypeVariable} 等组件深度联动，
 * 确保泛型解析的完整性、一致性和稳定性。
 *
 * <p>
 * 核心特性：
 ...
 * <li>多边界完整支持：聚合类+接口多边界（如 T extends Number & Comparable<T>），生成组合类型</li>
 * <li>嵌套泛型递归解析：支持边界中嵌套泛型变量（如 T extends List<U>），递归复用当前解析器</li>
