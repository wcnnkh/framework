...
import run.soeasy.framework.core.DefaultParameterNameDiscoverer;
import run.soeasy.framework.core.MethodParameter;
import run.soeasy.framework.core.ParameterNameDiscoverer;
...
public interface ParameterDescriptor extends PropertyDescriptor {
		...
		public TypeDescriptor getTypeDescriptor() {
			...
			if (typeDescriptor == null) {
				synchronized (this) {
					...
					if (typeDescriptor == null) {
						MethodParameter methodParameter = MethodParameter.forExecutable(source, index);
						typeDescriptors[index] = typeDescriptor = new TypeDescriptor(methodParameter);
					}
				}
			}
	...
	public static Elements<ParameterDescriptor> forExecutable(Executable executable) {
		return Elements.of(forExecutable(executable, PARAMETER_NAME_DISCOVERER));
	}
