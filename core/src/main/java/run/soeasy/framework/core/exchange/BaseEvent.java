...
import run.soeasy.framework.core.TimeUtils;
...
public class BaseEvent extends EventObject {
	 ...
	 * 基于现有事件创建新事件（浅拷贝）
	 * <p>
	 * 新事件将继承原始事件的ID和时间戳，仅修改事件源。 适用于需要保持事件链连续性的场景，如事件处理过程中的衍生事件。
	 * 
	 ...
	 * <p>
	 * 生成全局唯一的事件ID，并记录当前系统时间作为时间戳。 适用于事件源头的创建场景，如初始事件触发。
	 * 
	 ...
	 * <p>
	 * 新事件继承上下文事件的ID和时间戳，形成事件链关系， 适用于事件处理过程中需要追溯原始事件的场景。
	 * 
	 * @param source  事件源对象，不可为null
	...
	public String toString() {
		return TimeUtils.format(timestamp, "yyyy-MM-dd HH::mm::ss,SSS").orElse(null) + " on event " + id + " source "
				+ source;
	}
}