 ...
 * 转为{@link BigDecimal}：通过{@link #getAsBigDecimal()}实现，无精度丢失（如{@code BigInteger(123)}
 * → {@code BigDecimal(123.0)}）； -
 * 转为{@link String}：通过{@link #getAsString()}实现，输出格式与原生{@link BigInteger}一致（如负数{@code -456}
 * → {@code "-456"}）； -
 ...
 * BigDecimalValue compareDecimal = new BigDecimalValue(
 * 		new BigDecimal("123456789012345678901234567890123456789012345678901234567890500.0"));
 ...
 *         && bigIntValue.getAsBigInteger().compareTo(BigInteger.valueOf(Long.MIN_VALUE)) &gt;= 0) {
 * 	long longVal = bigIntValue.getAsBigInteger().longValueExact(); // 使用longValueExact()，溢出直接抛异常
 * } else {
 ...
 *
 * // 8. 取反操作（符号反转，无溢出风险）
 * BigIntegerValue positive = new BigIntegerValue(new BigInteger("12345678901234567890"));
 * BigIntegerValue negative = (BigIntegerValue) positive.negate();
 * System.out.println(negative.getAsString()); // 输出："-12345678901234567890"
 * </pre>
 *
 * <h3>注意事项</h3>
 * <ul>
 * <li>{@link #add(long)}方法修正：原逻辑未将累加结果赋值给{@link #value}，导致累加无效果；优化后通过{@code this.value = this.value.add(...)}更新状态，确保累加生效；</li>
 * <li>基础类型转换风险：若需转为{@code long}、{@code int}等基础类型，需先通过{@link BigInteger#compareTo(BigInteger)}校验范围（如{@code value.compareTo(BigInteger.valueOf(Long.MAX_VALUE)) &lt;= 0}），避免溢出；推荐使用{@link BigInteger#longValueExact()}、{@link BigInteger#intValueExact()}等带精确校验的方法，溢出时直接抛异常，便于问题定位；</li>
 * <li>单例常量不可修改：{@link #ZERO}和{@link #ONE}为单例实例，调用{@link #add(long)}或{@link #reset()}会修改其状态，建议仅作为“只读基准”使用，避免直接修改常量；若需修改，建议基于常量创建新实例（如{@code new BigIntegerValue(BigIntegerValue.ONE.getAsBigInteger())}）；</li>
 * <li>HTML转义说明：代码中的{@code <}需转为{@code &lt;}，{@code >}需转为{@code &gt;}（如{@code a < b}
 * → {@code a &lt; b}），避免HTML解析时被识别为标签；</li>
 * <li>空值防护：构造方法依赖{@link NumberUtils#newBigInteger(BigInteger)}做空值校验，若传入null会抛出{@link NullPointerException}，需确保输入参数非null。</li>
 * </ul>
 *
 * @author soeasy.run
...
public class BigIntegerValue extends NumberAdder {
     ...
     * <p>
     * 用途：计数累加的步长（如循环计数{@code one.add(1)}）、乘法运算的单位元（任何数×1不变）
     * <p>
     * <strong>注意</strong>：不可直接调用{@link #add(long)}修改其状态，否则会影响所有引用此常量的代码
     */
     ...
     * <p>
     * 核心逻辑： 1.
     * 接收外部传入的初始值，通过{@link NumberUtils#newBigInteger(BigInteger)}创建拷贝，避免外部引用影响； 2.
     * 将拷贝后的初始值赋值给{@link #initialValue}（永久存储，不可修改）； 3.
     * 调用{@link #reset()}方法，将{@link #value}初始化为{@link #initialValue}的拷贝，确保初始状态正确。
     *
     ...
     * <p>
     * 核心逻辑：将{@code long}值转为{@link BigInteger}，再调用主构造方法，避免外部手动转换
     *
     ...
     * <p>
     * 核心逻辑：将字符串转为{@link BigInteger}，再调用主构造方法；若字符串非法（如非数字、含小数），会抛出{@link NumberFormatException}
     *
     * @param initialValue 字符串类型初始值（如"12345678901234567890"、"-9876543210"，不可为null或空字符串）
     * @throws NumberFormatException 若字符串不是合法的整数格式
     ...
     * <p>
     * 比较规则（优先级从高到低）： 1. 若目标{@code o}为null：返回1（约定null小于任何数值类型）； 2.
     * 若目标{@code o}是数值类型（{@link Value#isNumber()}返回true）： -
     ...
     * 3.
     * 若目标{@code o}非数值类型：委托父类{@link NumberAdder#compareTo(Value)}处理（默认按类名哈希排序，确保排序一致性）。
     *
    ...
    /**
     * 判断当前实例与目标对象是否相等（遵循{@code equals-hashCode}契约，优化跨类型相等判定）
     * <p>
     * 相等规则： 1. 若目标对象与当前实例为同一引用：返回true； 2. 若目标对象为null：返回false； 3.
     * 若目标对象是{@link Value}接口实现类且为数值类型：比较两者的{@link BigInteger}值是否相等（支持跨类型数值相等，如{@link BigIntegerValue(123)}与{@link BigDecimalValue(123.0)}相等）；
     * 4. 其他情况：返回false。
     *
     ...
     * <p>
     * 转换逻辑：通过{@link BigDecimal#BigDecimal(BigInteger)}构造方法直接创建，
     * 数值与当前{@link #value}完全一致，小数部分为0（如{@code BigInteger(123)} →
     * {@code BigDecimal(123.0)}）。
     * <p>
     * 适用场景：需与{@link BigDecimalValue}（高精度小数）进行运算时，作为类型适配的中间步骤。
     *
     ...
     * <p>
     * 安全说明：由于{@link BigInteger}是不可变对象，返回当前{@link #value}的直接引用，
     ...
     * <p>
     * 转换逻辑：直接调用{@link #value}的{@link BigInteger#toString()}方法，
     * 输出格式与原生{@link BigInteger}一致（如正数{@code 456} → {@code "456"}，负数{@code -789} →
     * {@code "-789"}）。
     * <p>
     * 适用场景：日志打印、前端数值展示、JSON序列化（避免科学计数法）。
     *
     ...
     * <p>
     * 适用场景：循环复用实例（如线程池中的计数器），无需重新创建{@link BigIntegerValue}，降低对象创建开销。
     */
    @Override
    public void reset() {
     ...
     * <p>
     * 核心逻辑（修正后）： 1.
     * 将{@code long}类型的{@code value}转为{@link BigInteger}（通过{@link BigInteger#valueOf(long)}，无精度丢失）；
     * 2.
     * 调用当前{@link #value}的{@link BigInteger#add(BigInteger)}方法，生成新的累加后实例（因{@link BigInteger}不可变，原实例不变）；
     * 3. 将新实例赋值给{@link #value}，完成状态更新（修正原逻辑“未赋值导致累加无效果”的问题）。
     * <p>
     * 特殊场景：若{@code value}为负数，等价于“当前值减去{@code -value}”（如{@code add(-100)} →
     * 当前值-100）。
     *
     * @param value 要累加的{@code long}类型数值（可正可负，范围：{@code Long.MIN_VALUE} ~
     *              {@code Long.MAX_VALUE}）
     */
    @Override
    ...
    /**
     * 扩展累加方法：支持{@link BigInteger}类型数值累加（适配高精度输入场景）
     * <p>
     * 核心逻辑：直接调用{@link BigInteger#add(BigInteger)}生成新实例，更新{@link #value}
     *
    ...
    /**
     * 扩展累加方法：支持{@link BigIntegerValue}类型数值累加（跨实例高精度累加）
     * <p>
     * 核心逻辑：提取目标实例的{@link BigInteger}值，调用上述{@link #add(BigInteger)}方法
     *
    ...
    /**
     * 对当前数值取反（符号反转），无溢出风险（高精度特性）
     * <p>
     * 核心逻辑：通过{@link BigInteger#negate()}生成取反后的新实例，创建新的{@link BigIntegerValue}返回（原实例状态不变，符合不可变设计）
     * <p>
     * 特殊场景：数值0取反后仍为0（{@link BigInteger#negate()}特性）
     *
    ...
    /**
     * 重写toString方法，返回数值字符串（与{@link #getAsString()}一致，增强日志可读性）
     *
     * @return 当前数值的字符串表示（非null）
     */
    @Override
