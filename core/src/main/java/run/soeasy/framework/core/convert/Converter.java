...
/**
 * Converter接口定义了对象类型转换的标准API，是类型转换系统的核心接口。
 * 实现此接口的类负责将对象从一种类型转换为另一种类型。
 * 
 * <p>Converter接口提供了多种转换方法的重载形式，以支持不同场景下的类型转换需求。
 * 它还继承了Convertable接口以获取类型转换能力的判断功能。
 * 
