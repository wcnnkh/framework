...
import run.soeasy.framework.core.spi.TypeServiceMap;
 ...
 * 支持根据给定的源对象和目标类型描述符动态查找并调用匹配的映射工厂， 生成对应的映射实例。适用于需要根据不同目标类型采用不同映射策略的场景。
 *
 * <p>
 * <b>核心特性：</b>
 * <ul>
...
public class MappingProvider<K, V extends TypedValueAccessor, T extends Mapping<K, V>>
		extends TypeServiceMap<MappingFactory<Object, K, V, T>> implements MappingFactory<Object, K, V, T> {

	/**
	 * 根据源对象和目标类型描述符获取映射实例
	 * <p>
	 * 查找与目标类型兼容的第一个映射工厂，并使用该工厂创建映射实例。 若未找到匹配的工厂，返回null。
	 * 
	...
	public T getMapping(@NonNull Object source, @NonNull TypeDescriptor requiredType) {
		MappingFactory<? super Object, ? extends K, ? extends V, ? extends T> factory = assignableFrom(
	 ...
	 * 注册特定源类型的映射工厂
	 * <p>
	 * 将映射工厂注册到服务映射中，关联到指定的源类型。 注意：该方法使用了未检查的类型转换，调用者需确保类型安全。
	 * 
	 * @param <S>            源类型
