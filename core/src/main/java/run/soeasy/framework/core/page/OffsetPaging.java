...
/**
 * 基于偏移量的分页实现，通过偏移量（offset）和每页大小（pageSize）实现分页，偏移量作为Long型游标集成至游标分页体系。
 *
 * @author soeasy.run
 * @param <V> 分页内容的元素类型
 * @see CursorPaging
...
public class OffsetPaging<V> extends CursorPaging<Long, V> {
	 ...
	 * @param offset   偏移量，必须≥0（从0开始）
	 * @param pageSize 每页大小，必须&gt;0
	 * @param elements 原始数据列表，不可为null
	 * @return 内存分页后的OffsetPaging&lt;E&gt;实例
	 * @throws IllegalArgumentException 偏移量&lt;0或每页大小≤0时抛出
	 */
	 ...
	 * @param offset      偏移量，必须≥0（从0开始）
	 * @param pageSize    每页大小，必须&gt;0
	 * @param offsetQuery 分页查询器，不可为null；入参为偏移量、每页大小，返回元素集合Collection&lt;E&gt;
	 ...
	 * @param offset        偏移量，必须≥0（从0开始）
	 * @param pageSize      每页大小，必须&gt;0
	 * @param offsetQuery   分页查询器，不可为null；入参为偏移量、每页大小，返回原始查询结果T
	 * @param elementMapper 结果映射器，不可为null；将原始查询结果T转换为分页元素集合Collection&lt;E&gt;
	 * @param totalMapper   总数映射器，不可为null；将原始查询结果T转换为总记录数（Number类型）
	 * @return OffsetPaging&lt;E&gt;分页实例
	 * @throws IllegalArgumentException 偏移量&lt;0或每页大小≤0时抛出
	 */
	public static <T, E> OffsetPaging<E> of(long offset, int pageSize,
	 ...
	 * @param pageNumber      页码，必须≥1（从1开始）
	 * @param pageSize        每页大小，必须&gt;0
	 * @param pageNumberQuery 分页查询器，不可为null；入参为页码、每页大小，返回原始查询结果T
	 * @param elementMapper   结果映射器，不可为null；将原始查询结果T转换为分页元素集合Collection&lt;E&gt;
	 * @param totalMapper     总数映射器，不可为null；将原始查询结果T转换为总记录数（Number类型）
	 * @return OffsetPaging&lt;E&gt;分页实例
	 * @throws IllegalArgumentException 页码≤0或每页大小≤0时抛出
	 ...
	 * @param pageSize      每页大小，必须&gt;0
	 * @param offsetQuery   分页查询器，不可为null；入参为偏移量、每页大小，返回原始查询结果T
	 * @param elementMapper 结果映射器，不可为null；将原始查询结果T转换为分页元素列表Streamable&lt;V&gt;
	 * @param totalMapper   总数映射器，不可为null；将原始查询结果T转换为总记录数（Number类型）
	 * @throws IllegalArgumentException 偏移量&lt;0或每页大小≤0时抛出
	 */
