package run.soeasy.framework.core.function;

import java.util.function.Function;

import lombok.Getter;
import lombok.NonNull;
import lombok.RequiredArgsConstructor;

/**
 * 映射型可抛出异常的Runnable实现，支持将多个操作合并为一个逻辑单元，
 * 并在执行过程中进行异常类型转换和资源清理操作。
 * 该实现允许按顺序执行两个Runnable，并在执行前后分别进行异常处理和资源清理。
 *
 * <p>执行流程：
 * <ol>
 *   <li>执行compose操作</li>
 *   <li>执行andThen操作</li>
 *   <li>若执行过程中抛出异常，通过throwingMapper进行异常类型转换</li>
 *   <li>无论执行是否成功，最终执行endpoint进行资源清理</li>
 * </ol>
 *
 * <p>核心特性：
 * <ul>
 *   <li>操作组合：支持按顺序执行两个Runnable</li>
 *   <li>异常转换：可将原始异常类型E转换为目标异常类型R</li>
 *   <li>资源管理：确保无论执行结果如何，资源清理操作都会被执行</li>
 *   <li>不可变设计：所有依赖项在构造时注入，保证线程安全</li>
 * </ul>
 *
 * <p>使用场景：
 * <ul>
 *   <li>需要组合多个操作并统一异常处理的场景</li>
 *   <li>执行后必须释放资源的操作（如文件、网络连接等）</li>
 *   <li>需要将底层异常转换为业务异常的场景</li>
 *   <li>多步骤操作的事务性执行</li>
 * </ul>
 *
 * @param <E> 原始异常类型，即compose和andThen可能抛出的异常类型
 * @param <R> 目标异常类型，即最终抛出的异常类型
 * @see ThrowingRunnable
 */
@RequiredArgsConstructor
@Getter
class MappingThrowingRunnable<E extends Throwable, R extends Throwable> implements ThrowingRunnable<R> {
    
    /**
     * 前置操作，在主操作执行前执行。
     */
    @NonNull
    private final ThrowingRunnable<? extends E> compose;
    
    /**
     * 主操作，在前置操作执行后执行。
     */
    @NonNull
    private final ThrowingRunnable<? extends R> andThen;
    
    /**
     * 异常转换函数，将compose或andThen抛出的异常类型E转换为目标异常类型R。
     */
    @NonNull
    private final Function<? super E, ? extends R> throwingMapper;
    
    /**
     * 资源清理操作，无论执行成功或失败都会在finally块中执行。
     */
    @NonNull
    private final ThrowingRunnable<? extends R> endpoint;

    /**
     * 按顺序执行compose和andThen操作，并在执行前后分别进行异常处理和资源清理。
     * 执行流程：
     * <ol>
     *   <li>执行compose操作</li>
     *   <li>执行andThen操作</li>
     *   <li>若执行过程中抛出异常，通过throwingMapper进行异常类型转换</li>
     *   <li>无论执行是否成功，最终执行endpoint进行资源清理</li>
     * </ol>
     *
     * @throws R 执行过程中抛出的异常，经过throwingMapper转换后的类型
     */
    @SuppressWarnings("unchecked")
    @Override
    public void run() throws R {
        try {
            compose.run();
            andThen.run();
        } catch (Throwable e) {
            throw throwingMapper.apply((E) e);
        } finally {
            endpoint.run();
        }
    }
}