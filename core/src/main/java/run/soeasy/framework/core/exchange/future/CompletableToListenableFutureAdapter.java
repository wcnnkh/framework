...
public class CompletableToListenableFutureAdapter<T> implements ListenableFuture<T> {
     ...
     * <p>
     * 该构造函数会注册CompletableFuture的完成处理器，
     * 当CompletableFuture状态变更时，同步更新Stage的状态并触发事件。
     * 
     ...
     */
    @Override
    public Registration registerListener(@NonNull Listener<ListenableFuture<T>> listener) {
     ...
     * 阻塞获取任务结果（直到完成）
     * <p>
     * 该方法直接转发至CompletableFuture的get()方法，
     * 会阻塞当前线程直到任务完成或抛出异常。
     * 
     * @return 任务结果
