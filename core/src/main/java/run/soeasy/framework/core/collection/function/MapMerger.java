...
public class MapMerger<K, V> implements Merger<Map<K, V>> {
    ...
    /**
     * 单例实例，用于全局共享的Map合并器。
     * 该实例为无泛型参数的原始类型，可通过类型转换安全地用于任何键值类型。
     */
     ...
     * 将多个Map合并为一个Map。
     * 该方法执行以下操作：
     * 1. 遍历输入的Map集合，跳过null或空的Map
     * 2. 使用第一个非空Map创建结果Map，以保留其实现类型特性
     * 3. 按顺序将后续Map中的所有键值对添加到结果Map中（重复键会被覆盖）
     * 4. 如果没有找到非空Map，返回空Map
     *
