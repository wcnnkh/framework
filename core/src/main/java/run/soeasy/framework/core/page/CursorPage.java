 ...
 */
@Data
public class CursorPage<K, V> implements Pageable<K, V>, ListableWrapper<V, Listable<V>> {
	/** 当前页的游标，用于标识数据起始位置 */
	 ...
	 * @param source        分页数据集合（不可为null）
	 * @param nextCursor    下一页游标（无下一页则为null）
	 * @param totalCount    总记录数（未知则为null，需≥0）
	 */
	public CursorPage(K currentCursor, @NonNull Listable<V> source, K nextCursor, Long totalCount) {
