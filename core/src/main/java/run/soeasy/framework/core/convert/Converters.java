...
public class Converters extends ConfigurableServices<Converter> implements Converter {
	 ...
	 * <p>
	 * 使用{@link ConverterComparator#DEFAULT}对转换器进行排序， 确保转换器按优先级顺序尝试转换。
	 */
	public Converters() {
		super(ConverterComparator.DEFAULT);
	}
	...
	public Object convert(Object source, @NonNull TypeDescriptor sourceTypeDescriptor,
		...
		return filter((e) -> e.canConvert(sourceTypeDescriptor, targetTypeDescriptor))
				.map((e) -> e.convert(source, sourceTypeDescriptor, targetTypeDescriptor)).findFirst()
				.orElseThrow(() -> new ConverterNotFoundException(sourceTypeDescriptor, targetTypeDescriptor));
	}

	/**
	 * 判断是否存在匹配的转换器
	 * <p>
	 * 检查是否有任何转换器能处理从源类型到目标类型的转换， 适用于预先判断转换可行性的场景。
	 * 
	 * @param sourceTypeDescriptor 源类型描述符，不可为null
	 * @param targetTypeDescriptor 目标类型描述符，不可为null
