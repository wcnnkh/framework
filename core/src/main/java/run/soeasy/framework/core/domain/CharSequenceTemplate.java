 ...
 * 用于处理可分割的字符序列，支持版本号格式解析、数字转换和版本组合等功能。 该类支持通过分隔符将字符序列拆分为多个子序列，并提供统一的版本号操作接口。
 *
 * <p>
 * 核心特性：
 * <ul>
...
public class CharSequenceTemplate implements CharSequence, Version, Serializable {
	 ...
	 * 获取字符序列的元素集合，按分隔符分割。
	 * <p>
	 * 若分隔符为null，返回包含自身的单元素集合； 否则按分隔符分割为多个子模板。
	 *
	 * @return 分割后的元素集合
	 */
	@Override
	public Streamable<? extends CharSequenceTemplate> getAsElements() {
		if (delimiter == null) {
	 ...
	 * @return 分割后的元素集合
	 */
	public Streamable<? extends CharSequenceTemplate> getAsElements(CharSequence delimiter) {
		if (value == null) {
	 ...
	 * 判断字符序列是否可转换为数字。
	 * <p>
	 * 通过尝试调用{@link #getAsNumber()}进行验证， 若抛出异常则返回false。
	 *
	...
	public Version join(@NonNull Version version) {
		Streamable<Version> elements = getAsElements().map(Function.identity());
		elements = elements.concat(Streamable.singleton(version));
