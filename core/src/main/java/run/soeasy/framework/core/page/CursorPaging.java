 ...
 * 2. 游标定位：通过游标（K类型）标识分页位置，而非传统偏移量；
 * 3. 总数懒计算：总记录数优先取查询结果的已知总数，未知时遍历所有页累加计算； 
 * 4. 线程安全：双重检查锁定保证懒加载的线程安全性。
 * </p>
 * 
 * @author soeasy.run
...
public class CursorPaging<K, V> implements Paging<K, V>, SliceWrapper<K, V, Slice<K, V>> {
	 ...
	 * 分页查询器，不可为null。
	 * <p>
	 * 封装实际的分页查询逻辑，入参为游标和每页大小，返回分页结果Slice&lt;K, V&gt;。
	 * </p>
	 */
	@NonNull
	 ...
	 * </p>
	 * 
	 * @return 当前页的Slice&lt;K, V&gt;对象，永不返回null（无数据时返回空Slice）
	 */
	@Override
	 ...
	 * <p>
	 * 计算规则（优先级从高到低）： 
	 * 1. 若已通过{@link #setTotalCount(Long)}设置totalCount，直接返回该值； 
	 * 2. 若当前页查询结果包含已知总数（isKnownTotal=true），返回当前页的总数； 
	 * 3. 遍历所有分页，累加每一页的元素数量得到总记录数（性能较低，仅适用于总数未知场景）。
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * 判断规则： 
	 * 1. 若已手动设置totalCount（非null），返回true； 
	 * 2. 若当前页查询结果包含已知总数，返回true； 
	 * 3. 其他情况返回false（需遍历计算总数）。
	 * </p>
	 * 
	 ...
	 * <p>
	 * 新实例会继承当前实例的“总数是否已知”状态： 
	 * - 若当前实例已知总数（isKnownTotal=true），新实例直接复用该总数； 
	 * - 若当前实例未知总数，新实例totalCount为null。
	 * </p>
	 * 
	 * @param cursor   目标游标，作为新分页的起始位置
	 * @param pageSize 新分页的每页大小，必须&gt;0（不允许为0或负数）
	 * @return 新的CursorPaging&lt;K, V&gt;实例，继承当前实例的总数状态
