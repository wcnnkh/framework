...
/**
 * 数组类型解析器，用于表示和解析Java中的泛型数组类型， 继承自{@link AbstractResolvableType}并针对数组类型特性进行实现。
 * 该类封装了{@link GenericArrayType}接口，支持获取数组组件类型和原始类型信息， 适用于反射操作、泛型类型解析等场景。
 *
 * <p>
 * 核心特性：
 * <ul>
 ...
 */
@Getter
public class ArrayType extends RawType implements GenericArrayType {
	...
	public ArrayType(GenericArrayType genericArrayType, TypeVariableResolver typeVariableResolver) {
		this(Object[].class, genericArrayType.getGenericComponentType(), typeVariableResolver);
	}

	public ArrayType(Class<?> type, Type componentType, TypeVariableResolver typeVariableResolver) {
	...
	public ResolvableType getComponentType() {
		return this.componentType == null ? NONE
				: ResolvableType.forType(this.componentType, this.getTypeVariableResolver());
	}
	...
	@Override
	public final ResolvableType getGenericComponentType() {
