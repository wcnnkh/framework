 ...
 * <li>键值对集合的聚合映射（如Map&lt;String, List&lt;User&gt;&gt;与Map&lt;String, List&lt;UserDTO&gt;&gt;的转换）；</li>
 ...
 * List<MappingFilter<String, OrderValue, OrderMapping>> filters = Arrays.asList(
 *     new ZeroAmountFilter(), // 过滤金额为0的源值
 *     new KeyFormatCheckFilter() // 校验键格式（如用户ID必须为数字）
 * );
 * 
 * // 3. 创建MapMapper实例
 * MapMapper<String, OrderValue, OrderMapping> mapMapper = 
 *     new MapMapper<>(filters, valueMapper);
 * 
 * // 4. 准备源/目标上下文（包含键值对集合，如键=用户ID，值=订单列表）
 * MappingContext<String, OrderValue, OrderMapping> sourceContext = 
 *     new OrderMappingContext(sourceMap); // 源：Map<String, List<Order>>
 * MappingContext<String, OrderValue, OrderMapping> targetContext = 
 *     new OrderMappingContext(targetMap); // 目标：Map<String, List<OrderDTO>>（待填充）
 * 
 * // 5. 执行映射
...
public class MappedMapper<K, V extends TypedValueAccessor, T extends Mapping<K, V>> extends FilterableMapper<K, V, T> {
	 ...
	 * <p>适用于仅需执行过滤逻辑，无需核心映射（或通过子类重写映射逻辑）的场景
	 * 
	 ...
	 * <p>filters负责过滤逻辑，mapper负责值级核心映射，组合实现完整的键值对集合映射
	 * 
	 * @param filters 集合级与值级过滤器链，不可为null
	 ...
	 * <p>核心逻辑：验证上下文合法性 → 遍历目标键值对 → 聚合源同键值 → 逐个值映射 → 统计成功次数
	 * 
	...
	protected boolean doFilterableMapping(@NonNull MappingContext<K, V, T> sourceContext,
		...
		try(Stream<KeyValue<K, V>> targetStream = targetContext.getMapping().stream()){
			Iterator<KeyValue<K, V>> iterator = targetStream.iterator();
			while(iterator.hasNext()) {
				KeyValue<K, V> target = iterator.next();
				// 获取源映射中相同键的所有值
				Streamable<V> sourceElements = sourceContext.getMapping().getValues(target.getKey());
				try(Stream<V> sourceStream = sourceElements.stream()){
					Iterator<V> sourceIterator = sourceStream.iterator();
					while(sourceIterator.hasNext()) {
						V value = sourceIterator.next();
						if (super.doFilterableMapping(// 创建源上下文的当前键值对上下文
								sourceContext.current(KeyValue.of(target.getKey(), value)),
								// 创建目标上下文的当前键值对上下文
								targetContext.current(target), filters)) {
							count++;
						}
					}
				}
