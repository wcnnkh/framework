 ...
 * 该类继承自{@link ElementJoining}并实现{@link Splitter}接口， 兼具元素连接和分割的双向操作能力。
 *
 * <p>
 * 核心特性：
 * <ul>
...
public class ElementSplitter extends ElementJoining<Object> implements Splitter<String> {
	...
	/**
	 * 元素解码器，用于将分割后的字符串转换为目标类型。 该解码器在分割操作时应用，不可为null。
	 */
	@NonNull
	private final Function<? super CharSequence, ? extends String> decoder;

	public ElementSplitter(CharSequence delimiter, Function<? super Object, ? extends CharSequence> encoder,
			Function<? super CharSequence, ? extends String> decoder) {
		super(delimiter, encoder);
