...
public class DisposableDispatcher<T> extends AbstractChannel<T> implements ListenableChannel<T> {
	...
	/**
	 * 注册事件监听器，该监听器将在事件发布时仅被触发一次。 监听器会被添加到队列尾部，按FIFO顺序处理。
	 * 
	 * @param listener 待注册的监听器，不可为null
	 * @return 注册操作的回执，可用于取消注册
	 */
	@Override
	public Operation registerListener(@NonNull Listener<T> listener) {
		return registry.register(listener);
	}

	/**
	 * 同步发布事件并触发所有等待的监听器。 该方法会获取写锁，按FIFO顺序触发监听器，并在触发后立即移除。 即使某个监听器抛出异常，后续监听器仍会继续执行。
	 * 
	...
	/**
	 * 处理监听器执行过程中抛出的异常。 子类可重写此方法以实现自定义的异常处理逻辑。
	 * 
	...
	/**
	 * 清除所有注册的监听器。 此操作会释放所有未触发的监听器资源，后续发布的事件将不再触发任何监听器。
	 */
	public void clear() {
		registry.reset();
	}
