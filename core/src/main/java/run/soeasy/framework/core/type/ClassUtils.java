...
public class ClassUtils {
	...
	/**
	 * 常用类缓存，键为类名，值为对应的Class对象。主要用于高效反序列化远程调用。 包含基本类型、包装类型、常用类及其数组类型。
	 */
	...
	/**
	 * 基本类型名称映射，键为基本类型名称，值为对应的Class对象。 例如："int" -> int.class。
	 */
	private static final Map<String, Class<?>> primitiveTypeNameMap = new HashMap<String, Class<?>>(32);

	/**
	 * 基本类型到包装类型的映射，键为基本类型，值为对应的包装类型。 例如：int.class -> Integer.class。
	 */
	private static final Map<Class<?>, Class<?>> primitiveTypeToWrapperMap = new IdentityHashMap<Class<?>, Class<?>>(8);

	/**
	 * 包装类型到基本类型的映射，键为包装类型，值为对应的基本类型。 例如：Integer.class -> int.class。
	 */
	...
	/**
	 * 返回默认的类加载器：通常是线程上下文类加载器（如有）， 否则使用加载ClassUtils类的类加载器作为回退。
	 *
	...
	public static Paging<Class<?>, Class<?>> getInterfaces(@NonNull Class<?> sourceClass) {
		return new CursorPaging<Class<?>, Class<?>>(sourceClass, (clazz, size) -> {
			...
			return new Cursor<>(clazz, Listable.forCollection(list), clazz.getSuperclass(), null);
		});
	}

	/**
	 * 返回给定类的全限定名：对于数组类，返回组件类型名 + "[]"； 对于普通类，返回类名。
	 *
	 * @param clazz 类
	 * @return 类的全限定名
	 */
	...
	/**
	 * 获取基本类型对应的包装类，非基本类型直接返回自身（灵活兼容输入类型）。
	 * <p>
	 * 核心逻辑：
	 * <ul>
	...
	/**
	 * 根据JVM的基本类型命名规则解析给定的类名是否为基本类型。 也支持JVM内部的基本类型数组类名。
	 *
	...
	/**
	 * 判断提供的Class是否是JVM生成的lambda表达式或方法引用的实现类。 基于现代主流JVM的检查规则做出最佳努力判断。
	 *
	 * @param clazz 要检查的类
