 ...
 * 接收外部传入的转换器，从而在处理对象转换时使用该转换器。
 * 适用于需要动态配置转换器的场景，如数据处理组件、对象映射器等。
 * </p>
 *
 * <p><b>核心特性：</b>
 * <ul>
 ...
 * <p><b>潜在问题：</b>
 * <ul>
 *   <li>缺少获取转换器的方法（如{@link #getTransformer()}），可能导致设计不完整</li>
 *   <li>未定义转换器的线程安全策略，多线程环境中使用可能存在隐患</li>
 *   <li>未提供转换器的默认实现或空安全处理，调用前需确保转换器已设置</li>
 *   <li>接口无泛型参数，无法约束转换器操作的具体类型</li>
 * </ul>
 * </p>
 *
 * @author soeasy.run
...
public interface TransformerAware {
     ...
     * 设置用于对象转换的转换器。
     * <p>
     * 实现类应确保在接收到转换器后，后续的转换操作使用该转换器。
     * 建议在实现中添加空值校验，避免使用未初始化的转换器。
     * 
