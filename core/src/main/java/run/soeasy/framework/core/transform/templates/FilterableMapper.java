 ...
 * <p>核心设计逻辑：
 * 映射流程分为三个阶段：
 * 1. 过滤器链执行：按过滤器迭代顺序依次调用每个{@link MappingFilter}，完成上下文修改、数据验证等操作；
 * 2. 流程中断判断：若任一过滤器中断流程（如验证失败），则直接返回映射结果，不执行后续逻辑；
 * 3. 核心映射执行：所有过滤器正常执行完毕后，调用{@link #mapper}（基础映射器）完成最终数据映射。
 * 
 * <p><b>核心特性详解：</b>
 * <ul>
 ...
 * <li><strong>灵活可组合</strong>：支持两种组合方式：
 *     - 实例化时传入初始过滤器集合，固定基础映射逻辑；
 ...
 * List<MappingFilter<String, UserValue, UserMapping>> initialFilters = Arrays.asList(
 *     new UserDataValidateFilter(), 
 *     new DateFormatConvertFilter()
 * );
 * 
 * // 3. 创建可过滤映射器实例
 * FilterableMapper<String, UserValue, UserMapping> filterableMapper = 
 *     new FilterableMapper<>(initialFilters, baseMapper);
 * 
...
public class FilterableMapper<K, V extends TypedValueAccessor, T extends Mapping<K, V>> implements Mapper<K, V, T> {
	...
	/**
	 * 映射过滤器集合，不可为null
	 * <p>过滤器按迭代顺序依次执行，构成映射的预处理/后处理流水线；
	 * 支持包含预处理过滤器（如数据校验）与后处理过滤器（如结果增强），具体职责由过滤器实现类定义。
	 */
	@NonNull
	...
	/**
	 * 基础映射器，负责执行最终的核心映射逻辑
	 * <p>需在所有过滤器正常执行完毕后调用（若过滤器中断流程则不执行）；
	 * 若未设置（null），则过滤器链执行完毕后直接返回true（仅执行过滤逻辑，无核心映射）。
	 */
	 ...
	 * <p>执行流程：
	 * 1. 构建{@link ChainMapper}实例，封装初始过滤器链与基础映射器；
	 * 2. 调用ChainMapper执行过滤器链（按顺序调用每个过滤器）；
	 * 3. 过滤器链执行完毕后，调用基础映射器执行核心映射；
	 * 4. 返回整体映射结果（过滤器中断则返回中断时的结果，否则返回基础映射器的结果）。
	 * 
	 ...
	 * <p>与{@link #doMapping(MappingContext, MappingContext)}的区别：
	...
	public final boolean doMapping(@NonNull MappingContext<K, V, T> sourceContext,
		...
		Elements<MappingFilter<K, V, T>> elements = Elements.of(this.filters).concat(Elements.of(filters));
		return doFilterableMapping(sourceContext, targetContext, elements);
	}

	/**
	 * 可扩展的过滤映射执行方法（保护方法，供子类重写）
	 * <p>默认实现：通过{@link ChainMapper}调度过滤器链与基础映射器，子类可重写此方法以自定义过滤映射逻辑，
