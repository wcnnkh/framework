 ...
 * 
 * <p><b>与父类的核心差异：</b>
 * FilterableMapper专注于单一映射单元的处理，而ArrayMapper扩展为**集合维度的批量映射**，
 ...
 * List<MappingFilter<Long, OrderValue, OrderMapping>> filters = 
 *     Collections.singletonList(new InvalidStatusFilter());
 * 
 * // 3. 创建数组映射器实例
 * ArrayMapper<Long, OrderValue, OrderMapping> arrayMapper = 
 *     new ArrayMapper<>(filters, elementMapper);
 * 
 * // 4. 准备源/目标集合上下文（包含订单列表）
 * MappingContext<Long, OrderValue, OrderMapping> sourceContext = 
 *     new OrderMappingContext(sourceOrderList); // 源订单列表
 * MappingContext<Long, OrderValue, OrderMapping> targetContext = 
 *     new OrderMappingContext(targetOrderList); // 目标订单列表（待填充）
 * 
 * // 5. 执行集合映射
...
@Getter
@Setter
public class ArrayMapper<K, V extends TypedValueAccessor, T extends Mapping<K, V>> extends FilterableMapper<K, V, T> {
	 ...
	 */
	public ArrayMapper(@NonNull Iterable<MappingFilter<K, V, T>> filters) {
		super(filters);
	}

	/**
	 * 构造包含过滤器与内部映射器的ArrayMapper实例
	 * <p>filters负责过滤逻辑，mapper负责元素级核心映射，组合实现完整的集合映射流程
	 * 
	 * @param filters 集合级与元素级过滤器链，不可为null
	 * @param mapper 元素级核心映射器，负责单个元素的具体转换逻辑
	 */
	public ArrayMapper(@NonNull Iterable<MappingFilter<K, V, T>> filters, Mapper<K, V, T> mapper) {
		super(filters, mapper);
	}

	/**
	 * 重写父类方法，实现集合级批量映射逻辑
	 * <p>核心逻辑：验证上下文合法性 → 预处理源集合 → 遍历目标元素 → 匹配源元素 → 执行元素映射 → 清理已处理元素
	 * 
	...
	protected boolean doFilterableMapping(@NonNull MappingContext<K, V, T> sourceContext,
		...
		for (KeyValue<K, V> target : targetContext.getMapping().getElements()) {
			...
			while (sourceIterator.hasNext()) {
				...
				// 使用内部映射器处理元素映射
				if (super.doFilterableMapping(sourceContext.current(source), targetContext.nested(target), filters)) {
					// 映射成功后从源集合移除，避免重复处理
