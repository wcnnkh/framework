 ...
 * 
 * <p>
 * 核心机制： - 基于时间戳对比：通过资源的{@link Resource#lastModified()}方法获取最后修改时间，与上一次记录的时间戳对比；
...
public class ResourcePoller<T extends Resource> extends Poller {
	private static Logger logger = Logger.getLogger(ResourcePoller.class.getName());
		...
		synchronized (this) {
			...
			try {
				...
				logger.log(Level.WARNING, e,
						() -> "Failed to get lastModified for resource " + resource.getDescription());
			}
			// 对比时间戳并触发事件
