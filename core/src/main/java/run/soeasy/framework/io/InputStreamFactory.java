...
/**
 * 输入流工厂接口，用于创建具有特定处理流程的{@link InputStream}实例，
 * 并提供将输入流转换为{@link Reader}的功能。
 * 该接口扩展了{@link ReaderFactory}，支持通过流水线处理输入操作，
 * 并提供解码转换、通道适配等高级功能。
 *
 * <p><b>核心特性：</b>
 * <ul>
...
public interface InputStreamFactory<I extends InputStream> extends ReaderFactory<Reader> {
     ...
     * 为输入流添加指定字符集的解码转换。
     * <p>
     * 该方法返回一个新的输入流工厂，将原始输入流通过指定的字符集
     * 转换为目标Reader类型，使用{@link InputStreamReader}实现转换。
     * </p>
     * 
     ...
     * 为输入流添加指定字符集名称的解码转换。
     * <p>
     * 该方法返回一个新的输入流工厂，使用指定的字符集名称
     * 将输入流转换为{@link InputStreamReader}。
     * </p>
     * 
     ...
     * 为输入流添加自定义解码转换。
     * <p>
     * 该方法返回一个新的输入流工厂，将原始输入流通过指定的解码器函数
     * 转换为目标Reader类型，支持自定义的解码逻辑。
     * </p>
     * 
     ...
     * <p>
     * 此方法返回一个{@link InputStreamPipeline}实例，它将工厂提供的
     * 输入流管道应用于所有读取操作，实现统一的处理流程。
     * </p>
     * 
     ...
     * <p>
     * 该方法将输入流流水线映射为Reader流水线，使用{@link InputStreamReader}
     ...
     * 将当前输入流转换为{@link ReadableByteChannel}，
     * 适用于需要NIO通道操作的场景。
     * </p>
     * 
     * @return 可读字节通道
     ...
     * 将输入流内容传输到指定文件。
     * <p>
     * 该方法将当前输入流的全部内容传输到目标文件，自动处理
     * 资源的打开和关闭，确保数据传输完成后输入流被正确释放。
     * </p>
     * 
