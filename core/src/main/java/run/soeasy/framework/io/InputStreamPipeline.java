...
/**
 * 输入流流水线实现，作为{@link InputStream}的装饰器，
 * 提供线程安全的关闭操作和流水线处理能力。
 * 
 * <p>该类包装目标{@link InputStream}实例，通过{@link ThrowingRunnable}
 * 实现自定义关闭逻辑，并使用原子布尔值确保关闭操作的线程安全性。
 * 同时实现{@link Pipeline}接口，支持将输入流纳入流水线处理流程。
 * 
 * <p><b>核心特性：</b>
 * <ul>
 *   <li>装饰器模式：包装原始输入流并增强关闭行为</li>
 ...
 * <p><b>使用场景：</b>
 * <ul>
 *   <li>需要在关闭输入流时执行额外清理操作的场景</li>
 *   <li>多线程环境下确保输入流关闭操作的一致性</li>
 *   <li>构建输入流处理链并统一管理资源释放</li>
 * </ul>
 * 
 * @author soeasy.run
...
class InputStreamPipeline extends FilterInputStream implements Pipeline<InputStream, IOException> {
     ...
     * 基于流水线创建输入流包装实例。
     * <p>
     * 从流水线获取目标输入流，并将流水线的关闭操作作为自定义关闭逻辑。
     * 适用于需要将输入流纳入流水线处理的场景。
     * 
     * @param pipeline 输入流流水线，不可为null
     * @throws IOException 当从流水线获取输入流失败时抛出
     */
    ...
    /**
     * 基于目标输入流和自定义关闭逻辑创建包装实例。
     * <p>
     * 允许为输入流指定额外的关闭操作，该操作将在输入流关闭时执行。
     * 
     * @param in 目标输入流，不可为null
     ...
     * 
     * @return 目标输入流实例
     * @throws IOException 当获取输入流失败时抛出（通常不会发生）
     */
    @Override
     ...
     * 关闭输入流并执行自定义关闭逻辑。
     * <p>
     * 使用原子布尔值确保关闭操作仅执行一次，先关闭目标输入流，
     * 再执行自定义关闭逻辑，确保资源释放的顺序性。
     * 
     * @throws IOException 当关闭输入流或执行自定义逻辑时发生I/O错误
     */
    @Override
