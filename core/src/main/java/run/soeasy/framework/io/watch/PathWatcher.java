...
public class PathWatcher<T extends PathResource> extends Poller
	 ...
	 */
	@NonNull
	private final Publisher<? super Streamable<ChangeEvent<T>>> publisher;

	/**
	 * 线程工厂（非空），用于创建轮询线程，控制线程的命名、优先级、守护线程属性等， 确保监控线程的可管理性（如设置为守护线程避免阻塞JVM退出）。
	 */
	@NonNull
	 ...
	 * 
	 * <p>
	 * 通过内部注册中心{@link #registry}的映射操作，将注册的{@link PathPoller}转换为其关联的{@link T}资源，
	 ...
	 * <p>
	 * 执行流程： 1.
	 * 调用{@link WatchService#poll(long, TimeUnit)}获取文件系统事件键（{@link WatchKey}），超时则返回null；
	 * 2. 若获取事件时被中断（{@link InterruptedException}），直接返回（结束本次轮询）； 3.
	 * 遍历内部注册的所有{@link PathPoller}，调用其{@code run(Elements.singleton(watchKey))}处理事件；
	 * 4. 无论事件处理结果如何，在finally块中调用{@link PathPoller#run()}执行后续操作（如状态更新）。
	 * 
	 * <p>
	 * 注：此方法通过轮询持续获取事件，确保文件系统的变更能被及时捕获并处理。
	 */
	@Override
	public void run() {
		WatchKey watchKey;
		...
		registry.forEach((poller) -> {
			try {
				// 调用PathPoller的run方法处理当前WatchKey事件（包装为单元素集合）
				poller.run(Streamable.singleton(watchKey));
			} finally {
	 ...
	 * 
	 * <p>
	 * 通过调用{@link Poller#startEndlessLoop(long, TimeUnit, ThreadFactory)}启动轮询线程，
	...
	@Override
	public Operation register(T element) {
	...
	@Override
	public Operation deregister(T element) {
		return null;
	}
