 ...
 * 
 * <p>该类通过两个算法工厂分别管理签名生成算法（继承自{@link AlgorithmEncoder}）和签名验证算法，
...
public abstract class AlgorithmSigner<T> extends AlgorithmEncoder<T> {
    ...
    /**
     * 验证签名有效性（实现{@link BinaryEncoder#test}接口）
     * 
     * <p>处理逻辑：
     * 1. 通过{@link #verifyAlgorithmFactory}获取签名验证算法实例
     * 2. 调用抽象方法{@link #test}执行具体的签名验证逻辑
     * 3. 将安全相关异常转换为{@link CodecException}
     * 
