...
public class ByteArrayMessageConverter extends AbstractMessageConverter<byte[]> {
	...
	@Override
	protected void writeInternal(TypeDescriptor type, byte[] body, MimeType contentType, OutputMessage outputMessage)
			throws IOException, MessageConvertException {
		outputMessage.getOutputStream().write(body);
	}

	@Override
	protected Long getContentLength(byte[] body, MimeType contentType) throws IOException {
