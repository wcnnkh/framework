...
public class StandardObservableProperties<K, V> extends StandardObservableMap<K, V> {
	...
	public StandardObservableProperties(Map<K, V> sourceMap,
		...
		this(new StandardBroadcastEventDispatcher<>(), sourceMap, propertiesMapper);
	}

	public StandardObservableProperties(BroadcastEventDispatcher<ObservableChangeEvent<Map<K, V>>> eventDispatcher,
			Map<K, V> sourceMap, Function<? super Properties, ? extends Map<K, V>> propertiesMapper) {
