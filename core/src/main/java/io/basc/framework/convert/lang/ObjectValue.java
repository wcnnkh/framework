...
public interface ObjectValue extends Value, Optional<ObjectValue>, Wrapper<Object> {
	static final ObjectValue EMPTY = new EmptyValue();

	static final ObjectValue[] EMPTY_ARRAY = new ObjectValue[0];
	...
	static ObjectValue of(@Nullable Object value) {
		return of(value, null);
	}

	static ObjectValue of(@Nullable Object value, @Nullable TypeDescriptor type) {
		if (value == null && type == null) {
			return EMPTY;
		}

		if (type == null && value instanceof ObjectValue) {
			return (ObjectValue) value;
		}

		return new DefaultObjectValue(value, type);
	}
	...
	static Elements<ObjectValue> asElements(Object value, TypeDescriptor typeDescriptor) {
		if (value instanceof Collection) {
			Collection<?> collection = (Collection<?>) value;
			TypeDescriptor elementTypeDescriptor = typeDescriptor.getElementTypeDescriptor();
			return Elements.of(collection).map((v) -> ObjectValue.of(v, elementTypeDescriptor));
		} else if (value instanceof Iterable) {
			Iterable<?> iterable = (Iterable<?>) value;
			TypeDescriptor elementTypeDescriptor = typeDescriptor.getGeneric(0);
			return Elements.of(iterable).map((v) -> ObjectValue.of(v, elementTypeDescriptor));
		} else if (value instanceof Enumerable) {
			Enumerable<?> enumerable = (Enumerable<?>) value;
			TypeDescriptor elementTypeDescriptor = typeDescriptor.getGeneric(0);
			return Elements.of(enumerable).map((v) -> ObjectValue.of(v, elementTypeDescriptor));
		} else if (value.getClass().isArray()) {
			int len = Array.getLength(value);
			TypeDescriptor elementTypeDescriptor = typeDescriptor.getElementTypeDescriptor();
			return Elements.of(() -> IntStream.range(0, len)
					.mapToObj((index) -> ObjectValue.of(Array.get(value, index), elementTypeDescriptor)));
		} /*
			 ...
			 */
		return Elements.singleton(ObjectValue.of(value, typeDescriptor));
	}
	...
	default <E extends Throwable> Object convert(TypeDescriptor targetType,
		...
		while (true) {
			...
			if (source instanceof ObjectValue) {
				source = ((ObjectValue) source).getSource();
				sourceType = ((ObjectValue) source).getTypeDescriptor();
			}
			break;
		}
	...
	default BigDecimal getAsBigDecimal() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsBigDecimal();
		}
	...
	default BigInteger getAsBigInteger() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsBigInteger();
		}
	...
	default boolean getAsBoolean() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsBoolean();
		}
	...
	default byte getAsByte() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsByte();
		}
	...
	default char getAsChar() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsChar();
		}
	...
	default double getAsDouble() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsDouble();
		}
	...
	default <T extends Enum<T>> T getAsEnum(Class<T> enumType) {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsEnum(enumType);
		}

		return (T) convert(TypeDescriptor.valueOf(enumType), Converter.unsupported());
	}
	...
	default float getAsFloat() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsFloat();
		}
	...
	default int getAsInt() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsInt();
		}
	...
	default long getAsLong() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsLong();
		}
	...
	default Number getAsNumber() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsNumber();
		}
	...
	default <T> T getAsObject(Class<? extends T> type) {
		return (T) getAsObject(type, () -> convert(TypeDescriptor.valueOf(type), Converter.unsupported()));
	}
	...
	default Object getAsObject(TypeDescriptor type) {
		if (ObjectValue.isBaseType(type.getType())) {
	...
	default short getAsShort() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsShort();
		}
	...
	default String getAsString() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).getAsString();
		}
	...
	default boolean isNumber() {
		...
		if (value instanceof ObjectValue) {
			return ((ObjectValue) value).isNumber();
		}
	...
	default ObjectValue or(Object other) {
		return orElse(ObjectValue.of(other));
	}

	@Override
	default ObjectValue orElse(ObjectValue other) {
		return isPresent() ? this : other;
	}

	default <E extends Throwable> ObjectValue orGet(Source<? extends Object, ? extends E> other) throws E {
		return orElseGet(() -> ObjectValue.of(other.get()));
	}
}
