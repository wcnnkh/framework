...
public class TypeDescriptor implements AnnotatedElement, Serializable {
	 ...
	 */
	public TypeDescriptor(ResolvableType resolvableType, @Nullable Class<?> type,
	...
	public static TypeDescriptor map(Class<?> mapType, @Nullable ResolvableType keyType,
			@Nullable ResolvableType valueType) {
		Assert.notNull(mapType, "Map type must not be null");
		if (!Map.class.isAssignableFrom(mapType)) {
			throw new IllegalArgumentException("Map type must be a [java.util.Map]");
		}
