...
public class BDecoder implements BytesDecoder<Map> {
	...
	@Override
	public Map decode(InputStream source) throws IOException, DecodeException {
		BufferedInputStream data = new BufferedInputStream(source);
		try {
			Object res = decode(data, 0);
			if (res == null) {
	...
	public Object decode(InputStream bais, int nesting) throws IOException {
		...
		case 'd':
			...
			try {
				...
				while ((tempByteArray = (byte[]) decode(bais, nesting + 1)) != null) {

					// decode some more

					Object value = decode(bais, nesting + 1);
		...
		case 'l':
			...
			try {
				...
				while ((tempElement = decode(bais, nesting + 1)) != null) {
					// add the element
