...
public class BeanResolverExtendChain extends BeanResolverConfiguration implements BeanResolver {
	private final Iterator<? extends BeanResolverExtend> iterator;
	private final BeanResolver nextChain;

	public BeanResolverExtendChain(Iterator<? extends BeanResolverExtend> iterator) {
		this(iterator, null);
	}

	BeanResolverExtendChain(Iterator<? extends BeanResolverExtend> iterator, BeanResolver nextChain) {
	...
	public static BeanResolverExtendChain build(Iterator<? extends BeanResolverExtend> iterator) {
		return new BeanResolverExtendChain(iterator);
	}

	public static BeanResolverExtendChain build(Iterator<? extends BeanResolverExtend> iterator,
			BeanResolver nextChain) {
		return new BeanResolverExtendChain(iterator, nextChain);
	}
