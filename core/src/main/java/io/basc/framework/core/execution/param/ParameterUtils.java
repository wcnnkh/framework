...
public final class ParameterUtils {
	...
	public static ParameterDescriptor[] getParameterDescriptors(ParameterNameDiscoverer parameterNameDiscoverer,
		...
		for (int i = 0; i < names.length; i++) {
			...
			SimpleParameterDescriptor parameterDescriptor = new SimpleParameterDescriptor();
			parameterDescriptor.setPositionIndex(i);
			parameterDescriptor.setName(names[i]);
			parameterDescriptor.setTypeDescriptor(new TypeDescriptor(parameter));
			parameterDefinitions[i] = parameterDescriptor;
		}
		return parameterDefinitions;
	}

	public static ParameterDescriptor[] getParameterDescriptors(Executable executable) {
		return getParameterDescriptors(PARAMETER_NAME_DISCOVERER, executable);
	}
