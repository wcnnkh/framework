...
public class FactoryLoader {
	private static volatile Map<Scope, BeanFactory> FACTORY_MAP = new HashMap<>(4);
	private static final ReadWriteLock LOCK = new ReentrantReadWriteLock();

	@Nullable
	public static void bind(BeanFactory beanFactory) {
		Lock lock = LOCK.writeLock();
		lock.lock();
		BeanFactory source;
		try {
			source = FACTORY_MAP.put(beanFactory.getScope(), beanFactory);
		} finally {
			lock.unlock();
		}
	...
	public static <T> Optional<T> getBean(Class<? extends T> type) {
		BeanFactory beanFactory = getBeanFactory(type.getClassLoader());
		if (beanFactory == null) {
			return Optional.empty();
		}

		return Optional.of(beanFactory.getBean(type));
	}
	...
	public static Optional<Object> getInstance(String name, ClassLoader classLoader) {
		...
		return Optional.of(beanFactory.getBean(name));
	}
}
