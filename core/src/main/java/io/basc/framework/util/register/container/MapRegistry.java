...
public class MapRegistry<K, V, M extends Map<K, EntryRegistration<K, V>>> extends EntryRegistry<K, V, M>
	...
	public final V put(K key, V value) {
		...
		return write((map) -> {
			...
			if (old == null) {
				// created
				getEventPublishService().publishEvent(new ChangeEvent<>(keyValue, ChangeType.CREATE));
			} else {
				// update
				getEventPublishService().publishEvent(new ChangeEvent<>(keyValue, old));
			}
	...
	public final V remove(Object key) {
		return update((map) -> {
			...
			getEventPublishService().publishEvent(new ChangeEvent<KeyValue<K, V>>(registration, ChangeType.DELETE));
