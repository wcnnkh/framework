...
public class ReloadableSupplier<T> implements Supplier<T>, Reloadable {
	@NonNull
	private final Supplier<? extends T> supplier;
	@NonNull
	private final Publisher<? super ChangeEvent<T>> changeEventPublisher;
	...
	private void touchEvent(T parent, T current) {
		...
		if (parent == null) {
			...
			changeEvent = new ChangeEvent<T>(parent, current);
		}
		changeEventPublisher.publish(changeEvent);
	}
