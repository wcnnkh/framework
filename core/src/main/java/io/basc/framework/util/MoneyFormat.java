...
public final class MoneyFormat implements Codec<BigDecimal, String> {
	 ...
	 */
	public static final MoneyFormat CAPITALIZE = new MoneyFormat(
			NumberReplacer.CAPITALIZE);
	 ...
	 */
	public static final MoneyFormat LOWERCASE = new MoneyFormat(
			NumberReplacer.LOWERCASE);
	...
	public MoneyFormat(Codec<BigDecimal, String> numberCodec) {
		this(numberCodec, new NumberUnit[] { NumberUnit.MONEY_100000000,
				NumberUnit.MONEY_10000, NumberUnit.MONEY_1000,
				NumberUnit.MONEY_100, NumberUnit.MONEY_10, NumberUnit.MONEY },
				new NumberUnit[] { NumberUnit.MONEY_01, NumberUnit.MONEY_001 });
	}

	public MoneyFormat(Codec<BigDecimal, String> numberCodec,
			NumberUnit[] integerUnits, NumberUnit[] decimalUnits) {
	...
	public String encode(BigDecimal money) throws EncodeException {
		...
		if (decimals[0].compareTo(BigDecimal.ZERO) == 0) {
			...
			NumberUtils.format(sb, decimals[0], numberCodec::encode, 0,
					integerUnits.length, integerUnits);
		}
		...
		if (decimals[1].compareTo(BigDecimal.ZERO) == 0) {
			...
			NumberUtils.format(sb, decimals[1], numberCodec::encode, 0,
					decimalUnits.length, decimalUnits);
		}
		return sb.toString();
	}
	...
	public BigDecimal decode(String money) throws DecodeException {
		...
		if (index == -1) {
			...
			if (index == -1) {
				// 内嵌的
				return NumberUtils.parse(money, numberCodec::decode,
						ArrayUtils.merge(integerUnits, decimalUnits));
			} else {
				return NumberUtils.parse(money.substring(0, index),
						this::decode, integerUnits).add(
						NumberUtils.parse(money.substring(index + 1),
								this::decode, decimalUnits));
			}
		} else {
			// 是整数
			return NumberUtils.parse(money.substring(0, index), this::decode,
					ArrayUtils.merge(integerUnits));
		}
	}
