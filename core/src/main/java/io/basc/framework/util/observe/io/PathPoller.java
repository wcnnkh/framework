...
import io.basc.framework.util.observe.VariablePoller;
...
public class PathPoller<T extends FileVariable> extends VariablePoller<T> {
	...
	private final WatchKeyRegistry watchKeyRegistry = new WatchKeyRegistry(2, EmptyEventDispatcher.empty());

	public PathPoller(@NonNull T variable, @NonNull EventPublishService<ChangeEvent<T>> eventPublishService) {
		super(variable, eventPublishService);
	}
	...
	private void publishWatchEvents(Elements<WatchEvent<Path>> watchEvents) {
		Elements<ChangeEvent<T>> events = watchEvents.map(this::eventConvert).filter((e) -> e != null).toList();
		getEventPublishService().publishBatchEvents(events);
	}
