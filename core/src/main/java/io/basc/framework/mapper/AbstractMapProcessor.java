...
public abstract class AbstractMapProcessor<S, T, E extends Throwable> implements Processor<S, T, E> {
	...
	public T process(S source) throws E {
		...
		return (T) conversionService.convert(source, TypeDescriptor.forObject(source), typeDescriptor);
	}
	...
	protected <V> V mapEntity(S source, Class<V> targetType, Field parentField) throws E {
		V target = Sys.env.getInstance(targetType);
		for (Field field : fieldFactory.getFields(targetType, parentField).entity().all()) {
			Object value;
