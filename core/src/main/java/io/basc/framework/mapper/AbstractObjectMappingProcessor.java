...
public abstract class AbstractObjectMappingProcessor<S, T, E extends Throwable>
		implements Processor<S, T, E> {
	private ConversionService conversionService = Sys.env
			.getConversionService();
	private final TypeDescriptor typeDescriptor;
	private FieldFactory fieldFactory = MapperUtils.getFieldFactory();

	public AbstractObjectMappingProcessor(TypeDescriptor typeDescriptor) {
	...
	protected <V> V mapEntity(S source, Class<V> targetType, Field parentField)
			throws E {
		V target = Sys.env.getInstance(targetType);
		for (Field field : fieldFactory.getFields(targetType, parentField)
				.entity().all()) {
			Object value;
