...
public class DefaultFields implements Fields {
	...
	public Iterator<Field> iterator() {
		return fieldMetadatas.stream().map((metadata) -> createField(metadata)).iterator();
	}
	...
	public List<Field> getList() {
		return fieldMetadatas.getList().stream().map((metadata) -> createField(metadata)).collect(Collectors.toList());
	}
