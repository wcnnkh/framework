...
public interface MappingWriter {
	<T extends Element> void set(ObjectMapper objectMapper, Value source, Mapping<? extends T> sourceMapping,
			T sourceField, Value target, Mapping<? extends T> targetMapping, T targetField) throws MappingException;

	<T extends Element> void set(ObjectMapper objectMapper, ObjectAccess sourceField, Value target,
