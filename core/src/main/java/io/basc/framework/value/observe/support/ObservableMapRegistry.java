 ...
 * @param <V>
 */
public class ObservableMapRegistry<K, V> extends MergedObservable<Map<K, V>> {
	private final BroadcastEventDispatcher<ObservableEvent<Elements<K>>> keyEventDispatcher;
	/**
	 * 主节点，控制map类型和最高优先级数据
	 */
	private final ObservableMap<K, V> master;
	private final Function<? super Properties, ? extends Map<K, V>> propertiesMapper;

	public ObservableMapRegistry(ObservableMap<K, V> master,
	...
	public ObservableMapRegistry(Map<K, V> customMap,
			Function<? super Properties, ? extends Map<K, V>> propertiesMapper) {
		this(new ObservableMap<>(customMap), new StandardBroadcastEventDispatcher<>(), propertiesMapper);
	}
	 ...
	 * @return
	 */
	public BroadcastEventDispatcher<ObservableEvent<Elements<K>>> getKeyEventDispatcher() {
	 ...
	 * @return
	 */
	public ObservableMap<K, V> getMaster() {
		return master;
	}
