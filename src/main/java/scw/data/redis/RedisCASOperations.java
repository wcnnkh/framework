...
public class RedisCASOperations implements CASOperations {
	...
	private static final String CAS_EXP_SCRIPT = "local cas = redis.call('get', KEYS[2]) if cas == nil then cas = 0 end if KEYS[3] == cas  then redis.call('set', KEYS[1], ARGV[1], 'EX', KEYS[4]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1, 'EX', KEYS[4]) end return 1 else return 0 end";
	private static final String CAS_SCRIPT = "local cas = redis.call('get', KEYS[2]) if cas == nil then cas = 0 end if KEYS[3] == cas  then redis.call('set', KEYS[1], ARGV[1]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1) end return 1 else return 0 end";
	...
	private static final String ADD_EXP = "if redis.call('exists', KEYS[1]) == 1 then return 0 else redis.call('set', KEYS[1], ARGV[1], 'EX', KEYS[3]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1, 'EX', KEYS[3]) end return 0  end";
	private static final String ADD = "if redis.call('exists', KEYS[1]) == 1 then return 0 else redis.call('set', KEYS[1], ARGV[1]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1) end return 0  end";
	private static final String SET_EXP = "redis.call('set', KEYS[1], ARGV[1], 'EX', KEYS[3]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1, 'EX', KEYS[3]) end";
	private static final String SET = "redis.call('set', KEYS[1], ARGV[1]) if redis.call('exists', KEYS[2]) == 1 then redis.call('incr', KEYS[2], 1) else redis.call('set', KEYS[2], 1) end";
