...
public final class Result implements Serializable {
	...
	public <T> T getObject(Class<T> type, TableMapping tableMapping) {
		...
		if (type.isArray()) {
			return (T) getValues();
		} else if (scw.db.result.ResultSet.isOriginalType(type)) {
			if (values.length > 0) {
				return (T) values[0];
			}
			return null;
			...
			try {
				return (T) scw.db.result.ResultSet.wrapper(metaData, values, DB.getTableInfo(type), tableMapping);
	...
	public <T> T getObject(Class<T> type, String... tableNames) {
		...
		if (type.isArray()) {
			...
			try {
				return (T) scw.db.result.ResultSet.wrapper(metaData, values, DB.getTableInfo(type), tableNames);
			} catch (IllegalArgumentException e) {
				e.printStackTrace();
			} catch (IllegalAccessException e) {
				e.printStackTrace();
