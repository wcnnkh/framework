...
public final class DefaultAutoBeanService implements AutoBeanService {
		...
		if (autoConfig == null || autoConfig.service() == Object.class || autoConfig.service() == clazz) {
			...
			if (clazz.isInterface()) {
				...
				if(ClassUtils.isExist(name) && beanFactory.contains(name)){
					...
					name = index == -1? (clazz.getName() + "Impl"):(clazz.getName().substring(0, index) + ".impl." + clazz.getSimpleName() + "Impl");
					if(ClassUtils.isExist(name) && beanFactory.contains(name)){
						logger.info("{} reference {}", clazz.getName(), name);
						return new ReferenceAutoBean(beanFactory, name);
					}
	...
	private AutoBean createMemcached(BeanFactory beanFactory, PropertyFactory propertyFactory) throws Exception {
		if (ClassUtils.isExist("scw.data.memcached.x.XMemcached")) {
			...
			if (!StringUtils.isEmpty(host)) {
				...
				return new DefaultAutoBean(beanFactory, "scw.data.memcached.x.XMemcached",
	...
	private AutoBean createDB(BeanFactory beanFactory, PropertyFactory propertyFactory) throws Exception {
		...
		if (ResourceUtils.isExist(config)) {
			...
			if (ClassUtils.isExist("scw.db.DruidDB")) {
				...
				return new DefaultAutoBean(beanFactory, "scw.db.DruidDB", types, args);
	...
	private AutoBean createRedis(BeanFactory beanFactory, PropertyFactory propertyFactory) throws Exception {
		...
		if (ResourceUtils.isExist(config)) {
			...
			if (ClassUtils.isExist("scw.data.redis.jedis.RedisByJedisPool")) {
				...
				return new DefaultAutoBean(beanFactory, "scw.data.redis.jedis.RedisByJedisPool", types, args);
			}
		}
		return null;
	}
