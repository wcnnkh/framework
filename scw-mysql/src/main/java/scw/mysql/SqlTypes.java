...
public enum SqlTypes implements SqlType {
	 ...
	 */
	CHAR("char", String.class, 255L),
	/**
	 * 保存可变长度的字符串（可包含字母、数字以及特殊字符）。在括号中指定字符串的最大长度。最多 255 个字符。注释：如果值的长度大于
	 * 255，则被转换为 TEXT 类型。
	 */
	VARCHAR("varchar", String.class, 255L),
	/**
	 * 存放最大长度为 255 个字符的字符串。
	 */
	 ...
	 */
	BLOB("blob", Blob.class, 65535L),
	/**
	 * 存放最大长度为 16,777,215 个字符的字符串。
	 */
	MEDIUMTEXT("mediumtext", String.class, 4294967295L),
	/**
	 * 用于 BLOBs（Binary Large OBjects）。存放最多 16,777,215 字节的数据。
	 */
	MEDIUMBLOB("mediumblob", String.class, 4294967295L),
	/**
	 * 存放最大长度为 4,294,967,295 个字符的字符串。
	 */
	LONGBLOB("longblob", String.class, 4294967295L),
	/**
	 ...
	 */
	TINYINT("tinyint", Byte.class, 4),
	/**
	 * 带符号范围-32768到32767，无符号0到65535, size 默认为 6。
	 */
	SMALLINT("smallint", Short.class, 6),
	/**
	 * 带符号范围-8388608到8388607，无符号的范围是0到16777215。 size 默认为9
	 */
	MEDIUMINT("mediumint", Integer.class, 9),
	/**
	 ...
	 */
	BIGINT("bigint", Long.class, 20),
	/**
	 * 带有浮动小数点的小数字。在 size 参数中规定显示最大位数。在 d 参数中规定小数点右侧的最大位数。
	 */
	FLOAT("float", Float.class, 11),
	/**
	 * 带有浮动小数点的大数字。在 size 参数中规显示定最大位数。在 d 参数中规定小数点右侧的最大位数。
	 */
	DOUBLE("double", Double.class, 20),
	/**
	 * 作为字符串存储的 DOUBLE 类型，允许固定的小数点。在 size 参数中规定显示最大位数。在 d 参数中规定小数点右侧的最大位数。
	 */
	DECIMAL("decimal", String.class, 256),

	/***
	 * 即便 DATETIME 和 TIMESTAMP 返回相同的格式，它们的工作方式很不同。在 INSERT 或 UPDATE
	 * 查询中，TIMESTAMP 自动把自身设置为当前的日期和时间。TIMESTAMP 也接受不同的格式，比如
	 ...
	 * 至今的秒数来存储。格式：YYYY-MM-DD HH:MM:SS 注释：支持的范围是从 '1970-01-01 00:00:01' UTC 到
	...
	/**
	 * 2 位或 4 位格式的年。 注释：4 位格式所允许的值：1901 到 2155。2 位格式所允许的值：70 到 69，表示从 1970 到
	 * 2069。
	 */
	...
	SqlTypes(String name, Class<?> type, Integer maxLength) {
		this(name, type, maxLength == null ? null : maxLength.longValue());
	}

	SqlTypes(String name, Class<?> type, Long maxLength) {
		this.name = name;
		this.type = type;
