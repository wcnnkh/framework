...
public final class ResultFactoryActionInterceptor implements ActionInterceptor, ActionInterceptorAccept {
	...
	public boolean isAccept(HttpChannel httpChannel, Action action, ActionParameters parameters) {
		return getResultFactory(action) != null;
	}

	private ResultFactory getResultFactory(Action action) {
		return AnnotationUtils.getAnnotation(ResultFactory.class, action.getSourceClass(),
	...
	public Object intercept(HttpChannel httpChannel, Action action, ActionParameters parameters,
		...
		ResultFactory resultFactory = getResultFactory(action);
		if (resultFactory != null && resultFactory.enable()) {
			if (value != null && value instanceof Result) {
				return value;
			}
			return beanFactory.getInstance(resultFactory.value()).success(value);
		}
		return value;
	}
}
