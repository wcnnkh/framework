...
public class RabbitmqExchange extends AbstractRabbitmqExchange {
	...
	public RabbitmqExchange(Connection connection,
		...
		// 延迟消息路由
		this.delayExchangeDeclare = new ExchangeDeclare(DELAY_PREFIX
				+ getExchangeDeclare().getName());
		delayExchangeDeclare.setType(DELAY_EXCHANGE_TYPE);

		this.dixExchangeDeclare = new ExchangeDeclare(DIX_PREFIX
				+ getExchangeDeclare().getName());
		this.dixExchangeDeclare.setType(DELAY_EXCHANGE_TYPE);
		this.dixQueueDeclare = new QueueDeclare(dixExchangeDeclare.getName()
				+ ".queue");

		this.delayQueueDeclare = new QueueDeclare(
				delayExchangeDeclare.getName() + ".queue");
		// 设置延迟消息的死信队列(注意:
		// 不要给此队列绑定消费者，这样在消息过期后会进入死信队列，在死信队列中将消息发给指定队列那么就实现了消息的延迟发送)
		this.delayQueueDeclare.setArgument(X_DEAD_LETTER_EXCHANGE,
				dixExchangeDeclare.getName());
	}

	protected void checkName(String name) {
		Assert.requiredArgument(
				StringUtils.isNotEmpty(name)
						&& !name.startsWith(DIX_ROUTING_KEY)
						&& !name.startsWith(DELAY_ROUTING_KEY)
