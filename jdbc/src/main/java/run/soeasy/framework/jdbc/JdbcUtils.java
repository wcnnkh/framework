...
import run.soeasy.framework.jdbc.convert.ResultSetMapper;
 ...
 * 
 * <p>
 * 该类为工具类（{@link UtilityClass}），不可实例化，所有方法均为静态方法，操作可能抛出{@link SQLException}。
 * 
 * @author soeasy.run
 * @see ResultSet
...
public final class JdbcUtils {
	 ...
	 * <p>
	 * 查找逻辑： 1.
	 * 优先使用{@link ResultSetMetaData#getColumnLabel(int)}获取列的别名（SQL中AS指定的名称）； 2.
	 * 若别名为空，则使用{@link ResultSetMetaData#getColumnName(int)}获取列的实际名称。
	 * 
	 ...
	 * 
	 * <p>
	 * 处理逻辑： 1. 遍历参数数组，依次为PreparedStatement的占位符设置值（索引从1开始）； 2.
	 * 若参数为枚举（{@link Enum}），则转换为枚举的名称（{@link Enum#name()}）； 3.
	 * 其他类型直接通过{@link PreparedStatement#setObject(int, Object)}设置。
	 * 
	...
	private static volatile ResultSetMapper resultSetMapper;

	public static ResultSetMapper getResultSetMapper() {
