 ...
 * 是JDBC中参数化SQL执行的核心组件。
 * 
 * <p>该类在父类基础上强化了预编译语句的特性，支持参数绑定、查询结果集包装、批处理参数设置等，
...
public class PreparedStatementWrapped<T extends PreparedStatement> extends StatementWrapped<T> {
     ...
     * 
     * <p>调用{@link PreparedStatement#execute()}执行SQL，返回是否产生结果集（true表示有ResultSet，false表示仅有更新计数）。
     * 
     * @return 执行后是否存在结果集（true为有结果集，false为无结果集）
     * @throws SQLException 当SQL执行失败时抛出（如参数不匹配、语法错误等）
     */
    public boolean execute() throws SQLException {
        return optional().map(statement -> {
            try {
     ...
     * 
     * <p>调用{@link PreparedStatement#executeUpdate()}执行更新操作，返回受影响的行数。
     * 
     ...
     * 
     * <p>调用{@link PreparedStatement#executeQuery()}执行查询，将返回的{@link java.sql.ResultSet}包装为{@link ResultSetWrapped}，
     * 便于结果集的迭代与转换操作。
     * 
    ...
    public ResultSetWrapped query() {
        ...
        return new ResultSetWrapped(map(statement -> {
            try {
     ...
     * 
     * <p>通过{@link ThrowingConsumer}接收预编译语句，执行自定义操作（如设置fetchSize、超时时间等），
     ...
     * 
     * <p>通过{@link #after(ThrowingConsumer)}实现，内部调用{@link JdbcUtils#setParams(PreparedStatement, Object[])}，
     ...
     * 
     * <p>遍历输入的参数列表（每个元素为一个参数数组），为每个参数数组调用{@link #setParams(Object...)}绑定参数，
