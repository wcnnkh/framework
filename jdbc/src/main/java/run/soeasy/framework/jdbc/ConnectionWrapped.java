 ...
 * 
 * <p>
 * 该类通过包装连接的管道（{@link Pipeline}），将连接的操作转换为函数式管道调用，
...
public class ConnectionWrapped extends JdbcWrapped<Connection> {
	 ...
	 * <p>
	 * 功能： 1.
	 * 利用传入的{@link ThrowingFunction}从{@link Connection}创建目标Statement（如{@link PreparedStatement}）；
	 * 2. 为创建的Statement管道绑定关闭操作（调用{@link Statement#close()}）； 3.
	 * 返回包装后的{@link StatementWrapped}，支持链式的Statement操作。
	 * 
	...
	public <P extends Statement> StatementWrapped<P> createStatement(
		...
		Pipeline<P, SQLException> statementPipeline = map(function).onClose(Statement::close).closeable();
	 ...
	 * 
	 * <p>
	 * 通过调用{@link #createStatement(ThrowingFunction)}，使用{@link Connection#prepareStatement(String)}创建预编译语句，
	 ...
	 * 
	 * <p>
	 * 通过调用{@link #createStatement(ThrowingFunction)}，使用{@link Connection#prepareCall(String)}创建存储过程调用语句，
