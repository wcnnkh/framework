...
/**
 * JDBC结果集包装类，继承自{@link JdbcWrapped<ResultSet>}，用于包装{@link ResultSet}的管道（{@link Pipeline<ResultSet, SQLException>}），
 * 提供将结果集行数据转换为元素集合（{@link Elements<T>}）的核心方法，支持结果集的流式处理与行数据映射，
 * 是结果集数据提取与转换的关键组件。
 * 
...
public class ResultSetWrapped extends JdbcWrapped<ResultSet> {
    ...
    /**
     * 将结果集的行数据通过映射函数转换为{@link Elements<T>}集合
     * 
     * <p>核心功能：
     * 1. 创建{@link ResultSetIterator<T>}，将当前结果集管道与行映射函数关联，实现结果集的迭代；
     * 2. 通过{@link CollectionUtils#unknownSizeStream(CloseableIterator)}将迭代器转换为未知大小的流；
     * 3. 包装为{@link Elements<T>}，提供结果集行数据的集合式访问（支持迭代、流式处理等）。
     * 
     * @param <T> 结果集行转换后的目标类型
     * @param mapper 行映射函数，用于将{@link ResultSet}的当前行转换为T类型对象（可能抛出{@link SQLException}）
     * @return 包含转换后元素的{@link Elements<T>}实例，可用于遍历或流式处理结果集数据
     */
