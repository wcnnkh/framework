 ...
 * 提供将结果集行数据转换为元素集合（{@link Streamable}）的核心方法，支持结果集的流式处理与行数据映射，
 * 是结果集数据提取与转换的关键组件。
 * 
 * <p>该类通过包装结果集的管道，结合{@link ResultSetIterator}实现结果集的迭代，
 * 并通过{@link Streamable}提供统一的集合访问接口，简化从ResultSet到业务对象的转换流程，
...
public class ResultSetWrapped extends JdbcWrapped<ResultSet> {
    ...
    /**
     * 将结果集的行数据通过映射函数转换为{@link Streamable}集合
     * 
     * <p>核心功能：
     ...
     * 3. 包装为{@link Streamable}，提供结果集行数据的集合式访问（支持迭代、流式处理等）。
     * 
     * @param <T> 结果集行转换后的目标类型
     * @param mapper 行映射函数，用于将{@link ResultSet}的当前行转换为T类型对象（可能抛出{@link SQLException}）
     * @return 包含转换后元素的{@link Streamable}实例，可用于遍历或流式处理结果集数据
     */
